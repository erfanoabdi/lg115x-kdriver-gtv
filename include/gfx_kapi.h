...
#define GFX_IORW_GET_SURFACE_SETTING 	_IOW (GFX_IOC_MAGIC, 5, LX_GFX_SURFACE_SETTING_PARAM_T )
...
@def GFX_IOW_SYNC_SURFACE
    ...
    @param surface_id [IN] pointer to UINT32
    @return RET_OK if success, RET_ERROR otherwise.

    This IOCTL flushes all data caches related to the surface to the real memory.\n
    <b>2010/02/23 : </b> This IOCTL is not useless because OSD surface is mmapped with "non-cached"
                         attribute. This IOCTL will be removed someday.
...
*/
#define GFX_IOW_SYNC_SURFACE            _IOW ( GFX_IOC_MAGIC, 7, UINT32 )
#define	GFX_IORW_GET_SURFACE_MEM_INFO	_IOWR( GFX_IOC_MAGIC, 8, LX_GFX_SURFACE_MEM_INFO_PARAM_T )
//#define GFX_IOW_LOCK_SURFACE
//#define GFX_IOW_UNLOCK_SURFACE
 ...
 *
*/
#define GFX_IOW_BLEND					_IOW( GFX_IOC_MAGIC,  9, LX_GFX_MANUAL_BLEND_CTRL_PARAM_T )
#define GFX_IOW_MANUAL_BLEND			_IOW( GFX_IOC_MAGIC, 10, LX_GFX_MANUAL_BLEND_CTRL_PARAM_T )

/** set alpha conversion table for 16bpp surface
 *
 *	This IOCTL replaces 4 alpha level for alpha conversion between 16bit color to 32bit color
 *
 * @note This IOCTL seems rarely used.
*/
#define GFX_IOW_SET_ALPHA_CONV_TBL		_IOW( GFX_IOC_MAGIC, 11, LX_GFX_ALPHA_CONV_TBL_T )
 ...
 * @note This IOCTL seems rarely used.
*/
#define GFX_IOW_SET_CSC_CONV_TBL		_IOW( GFX_IOC_MAGIC, 12, LX_GFX_CSC_TBL_T )

/** get command delay between command queue request
 *
 */
#define GFX_IOR_GET_CMD_DELAY			_IOR( GFX_IOC_MAGIC, 13, UINT32 )
 ...
 */
#define GFX_IOW_SET_CMD_DELAY			_IOW( GFX_IOC_MAGIC, 14, UINT32 )
 ...
 *	@return OK
 */
#define	GFX_IOW_SET_BATCH_RUN_MODE		_IOW( GFX_IOC_MAGIC, 15, LX_GFX_BATCH_RUN_MODE_T )

/** get batch run mode
 *	@param batch_run_mode [OUT] batch run mode
 *	@return OK
 */
#define GFX_IOR_GET_BATCH_RUN_MODE		_IOR( GFX_IOC_MAGIC, 16, LX_GFX_BATCH_RUN_MODE_T )
 ...
 */
#define	GFX_IO_START_BATCH_RUN			_IO(  GFX_IOC_MAGIC, 17 )

/** set graphic sync mode 
 *
 *
 */
#define	GFX_IOW_SET_GRAPHIC_SYNC_MODE	_IOW( GFX_IOC_MAGIC, 18, LX_GFX_GRAPHIC_SYNC_MODE_T )
 ...
 *	@note
 */
#define GFX_IOW_WAIT_FOR_SYNC			_IOW( GFX_IOC_MAGIC, 19, UINT32 )

/** get gfx memory information
 *
 *
 */
#define GFX_IOR_GET_MEM_STAT			_IOR( GFX_IOC_MAGIC, 20, LX_GFX_MEM_STAT_T)

/** do the stratch blit 
 *
 *
 */
#define GFX_IOW_SW_SCALE				_IOW( GFX_IOC_MAGIC, 21, LX_GFX_SW_SCALE_CTRL_PARAM_T )
#define GFX_IOW_SW_BLIT					_IOW( GFX_IOC_MAGIC, 22, LX_GFX_SW_BLIT_CTRL_PARAM_T )
...
typedef struct
{
	UINT32					b_hw_scaler:1,				/**< is the current chip supports h/w scaler ? */
							xxx:31;
	UINT16					surface_blit_cmd_delay;		/**< default recommended value for cmd delay */
	UINT16					screen_blit_cmd_delay;		/**< default recommended value for cmd delay */

	UINT32					sync_wait_timeout;			/**< default recommanded timeout value for GFX sync */
	UINT8					sync_fail_retry_count;		/**< retry count when GFX timeout ocurrs */
 ...
 * @see GFX_IORW_GET_SURFACE_SETTING
 */
typedef struct
{
	UINT32						surface_fd;			/**< surface handle */
...
#define	LX_GFX_SW_SCALER_MODE_LINEAR		0x1000		/** simple linear scaler for speed up */
#define	LX_GFX_SW_SCALER_MODE_BILINEAR		0x2000		/** bilinear scaler for high quality */
	UINT32				scaler_mode;
}
LX_GFX_SCALE_CTRL_T, LX_GFX_SW_SCALE_CTRL_PARAM_T;
...
LX_GFX_SW_BLIT_CTRL_T, LX_GFX_SW_BLIT_CTRL_PARAM_T;
...
typedef struct
{
	LX_GFX_PHYS_MEM_INFO_T	mem_info_non_cached;	/**< memory info with non cached attribute  */
	LX_GFX_PHYS_MEM_INFO_T	mem_info_cached;		/**< memory info with cached attribute */
