	...
	switch(rw_type)
	{
		...
		case PE_ITEM_PKTRW(GET):
		{
			switch(module_type)
			{
				...
				break;
				case PE_ITEM_PKTMODL(D3D):
					if(rw_type==PE_ITEM_PKTRW(SET))	ret=PE_D3D_SetFunction(rcvdata,func_type,data_size);
					else							ret=PE_D3D_GetFunction(rcvdata,func_type,data_size);
				break;
				...
				case PE_ITEM_PKTMODL(DBG):
				{
					if(rw_type==PE_ITEM_PKTRW(SET))
					{
						do{
							...
							ret=PE_D3D_SetFunction(rcvdata,func_type,data_size);
...
int PE_IO_Init(unsigned int *data, unsigned int datasize)
{
	...
	do{
		...
		ret = PE_D3D_Init();
		PE_IO_CHECK_CODE(ret,break,"[%s,%d]PE_D3D_Init() error..\n",__F__,__L__);
