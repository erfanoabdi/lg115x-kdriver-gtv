...
int VENC_Init(void)
{
	...
	else
	{
		...
		OS_DEBUG_EnableModuleByIndex ( g_venc_debug_fd, 0, DBG_COLOR_NONE );
		OS_DEBUG_EnableModuleByIndex ( g_venc_debug_fd, 1, DBG_COLOR_NONE );
		OS_DEBUG_EnableModuleByIndex ( g_venc_debug_fd, 2, DBG_COLOR_NONE );
		OS_DEBUG_EnableModuleByIndex ( g_venc_debug_fd, 3, DBG_COLOR_NONE );
	}
    ...
    if ( request_irq(L9_IRQ_VENC0, (irq_handler_t)VENC_Interrupt, 0, "DTVSoC2VENC ", NULL)) {
        VENC_PRINT("request_irq in %s is failed\n", __FUNCTION__);
        return -EIO;
    }
	//VENC_PRINT("venc irq registered :%d:\n", IRQ_VENC0);
...
VENC_Ioctl ( struct file *filp, unsigned int cmd, unsigned long arg )
{
	...
	switch(cmd)
	{
		...
		case LX_VENC_IOW_ENC_SET_INPUT:
		{
			...
			if ( VENC_SetEncoderInput( &stParam ) != RET_OK )
			{
		...
		case LX_VENC_IOW_ENC_SET_QUALITY:
		{
			...
			if ( VENC_SetPictureQuality( eQuality ) )
			{
				ret = -EFAULT;
				break;
			}
		...
		case LX_VENC_IOW_ENC_SET_QUALITY_DEBUG:
		{
			...
			if ( copy_from_user( (void *)&ui32_QPValue, (void *)arg, sizeof(ui32_QPValue) ) )
            {
				VENC_PRINT("ioctl: copy_from_user error !!! \n");
 	            ret = -EFAULT;
				break;
			...
			if ( VENC_SetQPValue( ui32_QPValue ) )
			{
				ret = -EFAULT;
				break;
			}
		...
		case LX_VENC_IOW_ENC_GET_INFO:
		{
			...
			if( VENC_GetEncodingInfomation( &stParam ) != RET_OK )
			{
				ret = -EFAULT;
				break;
			}
		...
		case LX_VENC_IOW_SCL_START:
		{
			LX_VENC_SCL_START_T stParam;

			if ( copy_from_user( (void *)&stParam, (void *)arg, sizeof(LX_VENC_SCL_START_T) ) )
            {
				VENC_PRINT("ioctl: copy_from_user error !!! \n");
 	            ret = -EFAULT;
				break;
			...
			ret = VENC_StartScaler( &stParam );

			VENC_UNLOCK();

			if ( ret != RET_OK )
			{
				VENC_PRINT("Scale failed !!! \n");
 	            ret = -EFAULT;
				break;
			}
		...
		case LX_VENC_IOR_SCL_GET_IMAGE:
		{
			LX_VENC_SCL_IMAGE_T stParam;

			VENC_LOCK();

			ret = VENC_GetScaledImage( &stParam );
			...
			if ( copy_to_user( (void *)arg, (void *)&stParam, sizeof(LX_VENC_SCL_IMAGE_T) ) )
            {
				VENC_PRINT("ioctl: copy_to_user error !!! \n");
 	            ret = -EFAULT;
            }
		...
		case LX_VENC_IOR_ENC_GET_OUTPUT:
		{
			...
			if ( VENC_GetOutputBufferState( &stParam ) != RET_OK )
			{
 	            ret = -EFAULT;
				break;
			}
		...
		case LX_VENC_IOR_ENC_GET_TS_OUTPUT:
		{
#ifdef VENC_ENABLE_TS_OUT
			LX_VENC_ENC_TS_OUTPUT_T stParam;

			//Get the current input buffer status
			if ( VENC_GetOutputTSBufferState( &stParam ) != RET_OK )
			{
				ret = -EFAULT;
				break;
			}
			...
			/* After updating all the parameters in local structure, copy it back to user memory */
			if ( copy_to_user( (void *)arg, (void *)&stParam, sizeof(LX_VENC_ENC_TS_OUTPUT_T) ) )
			{
				VENC_PRINT("ioctl: copy_to_user error !!! \n");
				ret = -EFAULT;
			}
		...
		case LX_VENC_IOR_ENC_GET_TS_COUNT:
		{
			LX_VENC_ENC_TS_COUNT_T stParam;

			VENC_GetTsCounts( &stParam );

			/* After updating all the parameters in local structure, copy it back to user memory */
			if ( copy_to_user( (void *)arg, (void *)&stParam, sizeof(LX_VENC_ENC_TS_COUNT_T) ) )
            {
				VENC_PRINT("ioctl: copy_to_user error !!! \n");
 	            ret = -EFAULT;
            }
		...
		case LX_VENC_IOW_ENC_SET_FORCE_BP:
		{
			BOOLEAN enableForceBP;

			if ( copy_from_user( (void *)&enableForceBP, (void *)arg, sizeof(enableForceBP) ) )
            {
				VENC_PRINT("ioctl: copy_from_user error !!! \n");
 	            ret = -EFAULT;
				break;
			...
			ret = VENC_SetForceBP( enableForceBP );
		...
		case LX_VENC_IOR_ENC_GET_STC :
		{
			LX_VENC_ENC_STC_T stParam;

			VENC_GetSTC( &stParam );

			/* After updating all the parameters in local structure, copy it back to user memory */
			if ( copy_to_user( (void *)arg, (void *)&stParam, sizeof(LX_VENC_ENC_STC_T) ) )
			{
				VENC_PRINT("ioctl: copy_to_user error !!! \n");
				ret = -EFAULT;
			}
		...
		case LX_VENC_IOR_PREP_GET_STC:
		{
			UINT32	prepStc;

			VENC_PREP_GetSTC( &prepStc );

			/* After updating all the parameters in local structure, copy it back to user memory */
			if ( copy_to_user( (void *)arg, (void *)&prepStc, sizeof(UINT32) ) )
			{
				VENC_PRINT("ioctl: copy_to_user error !!! \n");
				ret = -EFAULT;
			}
		...
		case LX_VENC_IOR_GET_PREP_DE_STATUS:
		{
			LX_VENC_DE_STATUS_T stParam;

			VENC_PREP_GetDEStatus( &stParam );

			/* After updating all the parameters in local structure, copy it back to user memory */
			if ( copy_to_user( (void *)arg, (void *)&stParam, sizeof(LX_VENC_DE_STATUS_T) ) )
			{
				VENC_PRINT("ioctl: copy_to_user error !!! \n");
				ret = -EFAULT;
			}
...
VENC_Mmap ( struct file *pstFilePtr, struct vm_area_struct *pstVmArea )
{
	...
	offset += gpstVencMemConfig->uiSRAMBase;		// add venc base address
