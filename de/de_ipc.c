...
int DE_IPC_Init(void)
{
	...
	do {
		...
		if (lx_chip_rev() >= LX_CHIP_REV(L9,A0) ) {
			ret = DE_IPC_L9_Init(&addrVideoBuffToMCU, &addrVideoBuffFrMCU);
		}
		else
#endif
		{
			ret = DE_IPC_L8_Init(&addrVideoBuffToMCU, &addrVideoBuffFrMCU);
		}
		if (ret) break;
...
#ifdef USE_KDRV_CODES_FOR_L9
		if (lx_chip_rev() >= LX_CHIP_REV(L9,A0) ) {
			ret = DE_IPC_L9_Free(&addrVideoBuffToMCU, &addrVideoBuffFrMCU);
		}
		else
#endif
		{
			ret = DE_IPC_L8_Free(&addrVideoBuffToMCU, &addrVideoBuffFrMCU);
		}
		if (ret) break;
		gpVideoBuffToMCU = (char *)addrVideoBuffToMCU;
		gpVideoBuffFrMCU = (char *)addrVideoBuffFrMCU;
...
#endif
	{
		ret = VIDEO_IPC_WakeUpReg(VIDEO_IPC_MCU, TRUE);
	}
...
{
	return VIDEO_IPC_WakeUpReg(VIDEO_DMA_MCU, TRUE);
}
...
{
	return VIDEO_IPC_WakeUpReg(VIDEO_JPG_MCU, TRUE);
}
