...
int DE_DMA_L9_WakeUpReg(BOOLEAN turnOn)
{
	if (turnOn) {
		...
		DE_IPC_L9_FLWf(ext_intr_event, ack_interrupt_event_arm, 1);
#else
		DE_IPC_L9_FLWf(ext_intr_event, ipc_interrupt_event_arm, GET_PMSK(28, 1));
#endif
	} else {
#ifdef USE_IPC_CONTROL_INTERRUPT_A_BIT
		DE_IPC_L9_FLWf(ext_intr_clear, ack_interrupt_clear_arm, 1);
#else
		DE_IPC_L9_FLWf(ext_intr_clear, ipc_interrupt_clear_arm, GET_PMSK(28, 1));
#endif
	}
...
#if (KDRV_PLATFORM != KDRV_COSMO_PLATFORM)  // bs.seong 20130207
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_REG_L9_OIF_SetLvdsPolarity(LX_DE_OIF_LVDS_POLARITY_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		DE_OIF_L9_RdFL(sync_de_control);
		DE_OIF_L9_Wr01(sync_de_control, hsync_pol, pstParams->hPolarity);
		DE_OIF_L9_Wr01(sync_de_control, vsync_pol, pstParams->vPolarity);
		DE_OIF_L9_WrFL(sync_de_control);
	} while (0);
...
int DE_REG_L9_SetMultiWinSrc(LX_DE_MULTI_WIN_SRC_T *pstParams)
{
	...
	do {
		...
		if ( lx_chip_rev() >= LX_CHIP_REV(L9,B0) ) {
			...
			switch (multiSrc)
			{
				case LX_DE_MULTI_IN_ADC_CVD  :
				case LX_DE_MULTI_IN_HDMI_CVD :
				case LX_DE_MULTI_IN_MVI_CVD  :
					CTOP_CTRL_L9B_RdFL(ctr26_extclk_div_sel);
					CTOP_CTRL_L9B_Wr01(ctr26_extclk_div_sel, cvbs54_sel, 1);
					CTOP_CTRL_L9B_WrFL(ctr26_extclk_div_sel);
				break;
				default :
					CTOP_CTRL_L9B_RdFL(ctr26_extclk_div_sel);
					CTOP_CTRL_L9B_Wr01(ctr26_extclk_div_sel, cvbs54_sel, 0);
					CTOP_CTRL_L9B_WrFL(ctr26_extclk_div_sel);
				break;
			}
		} else {
