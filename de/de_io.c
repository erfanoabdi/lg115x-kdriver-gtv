 ...
 *   Control Constants
 *---------------------------------------------------------------------------------------*/
#undef  USE_GRAB_PIXEL_BY_CAPTURING_FRAME
...
typedef enum {
    ...
    DE_SUSPEND_SETLVDSPOLARITY      ,  // 51
    DE_SUSPEND_SETMULTIVISION          // 52
} DE_SUSPEND_INDX_T;
...
DE_SUSPEND_STORE_T de_suspend_store[] = {
    ...
    {TRUE  ,DE_SUSPEND_SETLVDSPOLARITY		,sizeof(LX_DE_OIF_LVDS_POLARITY_T) 	,DE_IO_OIF_SetLvdsPolarity	,NULL},
...
extern LX_VDEC_MEM_CFG_T   *gpVdecMem;
#if (KDRV_PLATFORM != KDRV_COSMO_PLATFORM)
...
LX_DE_DIS_FMT_T		g_de_DisFmt = { .panel_type = LX_PANEL_TYPE_MAX, .dev_type = LX_PANEL_DEV_MAX };
...
int DE_IO_SetCviSrcType(unsigned long arg)
{
    ...
    do {
        ...
        if (ret) break;
		g_de_CviSrcType.stability = gpVdecMem->dpb.mem_base;
...
int DE_IO_SetDisFmt(unsigned long arg)
{
    ...
    do {
		...
		/* save active display format info */
		memcpy( &g_de_DisFmt, &stParams, sizeof(LX_DE_DIS_FMT_T) );
 ...
 * @callergraph
 *
 * @brief get current panel information 
 *
 * @param arg [OUT] address of buffer to be carried
 *
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_IO_GetDisFmt(unsigned long arg)
{
//	LX_DE_DIS_FMT_T *pstParams = (LX_DE_DIS_FMT_T*)arg;

	/*  check if panel is configured or not */
	if ( g_de_DisFmt.panel_type >= LX_PANEL_TYPE_MAX || g_de_DisFmt.dev_type >= LX_PANEL_DEV_MAX ) return RET_ERROR;

	memcpy( (LX_DE_DIS_FMT_T*)arg, &g_de_DisFmt, sizeof(LX_DE_DIS_FMT_T) );
	return RET_OK;	
}
...
int DE_IO_SetCviVideoFrameBuffer(unsigned long arg)
{
    ...
    do {
        ...
        phyFrameBuffer = GET_BITS(frameInfo.region.pixelGrabY ,16,16);
        pFrameColor = (LX_DE_GRAB_COLOR_T *)ioremap(VIDEO_L9_ROW2PHY(phyFrameBuffer), frameSize);
...
int FMT3D_SetLAMode(BOOLEAN b3dLaMode)
{
    int ret = RET_OK;
	LX_DE_CVI_SRC_TYPE_T cviSrcType;

    do {
		cviSrcType = g_de_CviSrcType;
		if (b3dLaMode) cviSrcType.sampling = LX_DE_CVI_DOUBLE_SAMPLING;
		cviSrcType.cvi_channel = LX_DE_CVI_CH_A;
        ret = DE_HAL_SetCviSrcType(&cviSrcType);
		if (ret) break;
		cviSrcType.cvi_channel = LX_DE_CVI_CH_B;
        ret = DE_HAL_SetCviSrcType(&cviSrcType);
    } while (0);
 ...
 * @return RET_OK(0) if success, none zero for otherwise
 */
int DE_IO_OIF_SetLvdsPolarity(unsigned long arg)
{
    int ret;
    LX_DE_OIF_LVDS_POLARITY_T stParams;
#ifndef USE_DE_SUSPEND
    LX_DE_OIF_LVDS_POLARITY_T *pstParams = (LX_DE_OIF_LVDS_POLARITY_T*)arg;
#endif

    do {
        ret = DE_COPY_FROM_USER(DE_SUSPEND_SETLVDSPOLARITY, &stParams, (void __user *)arg, sizeof(stParams));
        if (ret) break;
        ret = DE_HAL_OIF_SetLvdsPolarity(&stParams);
    } while (0);
