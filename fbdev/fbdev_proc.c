...
----------------------------------------------------------------------------------------*/
static void	_FBDEV_PrintOSDInfo ( struct seq_file *m );
...
#if(KDRV_PLATFORM == KDRV_COSMO_PLATFORM)
	{ "scrclear",	PROC_ID_SCREEN_CLEAR, OS_PROC_FLAG_WRITE},
#endif
	{ NULL, 		PROC_ID_MAX		, 0 }
};
...
//			len += sprintf( buffer + len, "[2] sa = %x\n", g_fbdev_reg_l8->osd2_sc_cfg.osd2_sc_bc_sa );
		...
		case PROC_ID_SYNC_CTRL:
		{
			...
			len += sprintf( buffer + len, "OSD 0 : %d\n", g_fbdev_cfg->fb[0].b_use_vsync );
			len += sprintf( buffer + len, "OSD 1 : %d\n", g_fbdev_cfg->fb[1].b_use_vsync );
			len += sprintf( buffer + len, "OSD 2 : %d\n", g_fbdev_cfg->fb[2].b_use_vsync );
			len += sprintf( buffer + len, "OSD 3 : %d\n", g_fbdev_cfg->fb[3].b_use_vsync );
			len += sprintf( buffer + len, "\n");
...
//			g_fbdev_reg_l8->osd2_sc_cfg.osd2_sc_bc_sa = value;
		...
		case PROC_ID_SYNC_CTRL:
		{
			...
			if ( osd_id >= 0 && osd_id < LX_FBDEV_ID_MAX )
			{
				g_fbdev_cfg->fb[osd_id].b_use_vsync = on_off_val;
			}
		}
		break;
		...
		case PROC_ID_SCREEN_CLEAR:
		{
			...
			if ( val > 4 ) return -1;
			{
				...
				loop = g_fbdev_cfg->fb[val].max_buf_num;

				if ( ( val ==4 ) && ( loop > 2 ) ) loop = 2;		// for COSMO osd0 plain.

				for ( i  =0 ; i < loop ; i++ )
				{
					FBDEV_RunGFXClearSurface(gMemCfgFBDev[val].mem_frame_base + g_fbdev_cfg->fb[val].max_xres*g_fbdev_cfg->fb[val].max_yres*4*i , g_fbdev_cfg->fb[val].max_xres ,g_fbdev_cfg->fb[val].max_yres  , 0 );
				}
		...
		break;
#endif //(KDRV_PLATFORM == KDRV_COSMO_PLATFORM)
		default:
		{
			/* do nothing */
		}
...
static void	_FBDEV_PrintOSDInfo ( struct seq_file *m )
{
	...
	for ( i=0; i<4 ; i++ )
	{
		struct dm_win_info* w = g_dm_ctx->winfo[i];
		seq_printf( m, "[ OSD %d ]\n", w->fb_dev_id );
		...
		seq_printf( m, " - pixel.eOSDPixFmt      = 0x%x\n", (UINT32)w->pixel.eOSDPixFmt );
		seq_printf( m, " - pixel.eOSDPixDepth    = 0x%x\n", (UINT32)w->pixel.eOSDPixDepth );
