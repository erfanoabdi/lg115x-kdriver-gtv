 ...
 * 
 */
void FBDEV_L9A0_InitHAL ( void )
{
	PRINTK("^g^[fb] L9A0(A1) HAL....\n");
...
static void FBDEV_L9A0_MergePOSD ( int layer, BOOLEAN b_flip )
{
	...
	if ( merge_surf_list[0] < 0 )	merge_surf_list[0]	= FBDEV_GFX_CreateSurface( 1280, 720, g_frc_osd_emul.osd_addr[0] );
	if ( merge_surf_list[1] < 0 )	merge_surf_list[1]	= FBDEV_GFX_CreateSurface( 1280, 720, g_frc_osd_emul.osd_addr[1] );
	 ...
	 */
	{
		...
		memcpy ( (void*)posd3_canvas->uiOSDHeaderPTR, (void*)&osd_hdr, sizeof(FBDEV_OSD_HDR_T) );
	}
