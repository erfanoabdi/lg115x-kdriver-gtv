...
int FBDEV_L9_InitOSDLayer ( int layer )
{
    ...
    {
        osd_hdr_addr = (( ((gMemCfgFBHdr[layer].mem_header_base))&0x0FFFFFFF)| gpFbdevRegCfg->slkrd_prefix_mask );
    }
    else
    {
 ...
 *
 * @param addr [IN] phsical address to hold palette data
 * @param size [IN] palette size
 * @param data [IN] palette data
 *
 *	@return RET_OK if success, RET_ERROR otherwise
 *
 */
int FBDEV_L9_DownloadPalette(unsigned int addr , unsigned int size , void *data)
{
	int ret = 0;
	unsigned long base;
...
========================================================================================*/
void FBDEV_L9_ISRTickHandler ( UINT32 int_cnt )
{
