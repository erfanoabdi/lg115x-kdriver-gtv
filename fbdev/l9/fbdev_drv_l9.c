...
extern int FBDEV_L9_DownloadPalette     (unsigned int addr , unsigned int size , void *data);
...
extern void FBDEV_L9_ISRTickHandler		(UINT32 int_cnt);
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static	FBDEV_HAL_T		g_fbdev_hal_l9 =
{
	...
	.DownloadPalette	= FBDEV_L9_DownloadPalette,
	...
	.ISRTickHandler		= FBDEV_L9_ISRTickHandler,
};
...
/** initialize L9 specific HAL.
 *
 *	@note this function is called before any L9 configuration
 */
void	FBDEV_L9_InitHAL( FBDEV_HAL_T*	hal )
{
	memcpy( hal, &g_fbdev_hal_l9, sizeof(FBDEV_HAL_T));
...
int FBDEV_L9_InitHW		(void)
{
	...
	/* access to register */
	g_fbdev_l9_reg =(OSD_L9A0_REG_T *)ioremap( gpFbdevRegCfg->reg_base_addr, gpFbdevRegCfg->reg_size );
	FBDEV_CHECK_CODE( g_fbdev_l9_reg == NULL, goto func_exit, "out of memory\n");

	g_fbdev_l9_reg_cached = (OSD_L9A0_REG_T *)OS_Malloc( gpFbdevRegCfg->reg_size );
...
//	OSD_L9A0_RdFL(osd_mixer_out_right);
	...
	g_fbdev_l9_hw_func.L9_InitHAL( );

	/* raxis.lim (2011/12/14) 
	 * ajudst GPU clock after initializaint L9 HAL 
