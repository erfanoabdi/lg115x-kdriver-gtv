...
/** initialize L9A0(A1) HAL & Cfg 
 *
 * 
 */
void FBDEV_L9B0_InitHAL ( void )
{
	PRINTK("^g^[fb] L9B0 HAL....\n");
...
//				PRINTK("^g^TRID MODE = TB (%d,%d)\n", osd_hdr.type.osd.osd_hdr_w_mem, osd_hdr.type.osd.osd_hdr_h_mem );
			...
			if( g_fbdev_frc_ctrl.b_frc_720p_disp_enable )
			{
				 ...
				 * surface should be cleared and copy region should be as small as possible to void the garbage.
			 	 */
				int	crsr_surf = FBDEV_GFX_CreateSurface( 128, 128, bitmap_ptr );
#if 0
				FBDEV_GFX_ClearSurface  ( g_frc_osd_emul.crsr_1280x1080_surface, 128, 128 );
				FBDEV_GFX_StretchSurface( crsr_surf, g_frc_osd_emul.crsr_1280x1080_surface, 0, 0, 128, 128, 0, 0, 88, 128 );
#else
				FBDEV_GFX_ClearSurface  ( g_frc_osd_emul.crsr_1280x1080_surface, 128, 128 );
				FBDEV_GFX_StretchSurface( crsr_surf, g_frc_osd_emul.crsr_1280x1080_surface, 0, 0, csr_x_size, csr_y_size, 0, 0, csr_x_size*2/3, csr_y_size );
#endif
				FBDEV_GFX_FreeSurface( crsr_surf );
			...
			bitmap_ptr += ( csr_y_off * hCanvas->stride ) + ( csr_x_off * (pixel_depth_2_bpp[hCanvas->eOSDPixDepth]>>3) );
...
int     FBDEV_L9B0_SetConnCtrl           (LX_FBDEV_CONN_CTRL_T* pConnCtrl)
{
    ...
    else if ( pConnCtrl->conn_type == LX_FBDEV_CONN_STD_FRCIII_SEPARATE )
    {
		...
		if ( g_frc_osd_emul.crsr_1280x1080_surface < 0 )
		{
			...
			g_frc_osd_emul.crsr_1280x1080_surface   = FBDEV_GFX_CreateSurface(128,192/*256*/, 0x0 );

			FBDEV_GFX_ClearSurface( g_frc_osd_emul.crsr_1280x1080_surface, 128, 192/*256*/ );

			FBDEV_GFX_GetSurfaceInfo(g_frc_osd_emul.crsr_1280x1080_surface, &surface_info );
...
static void FBDEV_L9B0_MergePOSD_720p( int layer, BOOLEAN b_flip )
{
    ...
    /* create external surface for POSD merge */
    {
        ...
        {
            int surf = FBDEV_GFX_CreateSurface( 128, 128, 0x0 );
            FBDEV_CHECK_CODE( surf<0, goto func_exit, "can't create merge surface\n");

            FBDEV_GFX_GetSurfaceInfo ( surf, &surface_info );
        ...
        {
            int surf = FBDEV_GFX_CreateSurface( 1280, 720, 0x0 );
            FBDEV_CHECK_CODE( surf<0, goto func_exit, "can't create merge surface\n");
	...
	memcpy( (void*)&osd_hdr, (void*)&g_frc_osd_emul.osd_hdr_sav[0], sizeof(FBDEV_OSD_HDR_T) ); 

	if ( crsr_surf < 0 )			crsr_surf			= FBDEV_GFX_CreateSurface(  88,   88, g_frc_osd_emul.crsr_addr );		 /* 88x88 */
	if ( crsr_bkup_surf < 0 )		crsr_bkup_surf		= FBDEV_GFX_CreateSurface(  88,   88, g_frc_osd_emul.crsr_bkup_addr );	 /* 88x88 */
	if ( merge_surf_list[0] < 0 )	merge_surf_list[0]	= FBDEV_GFX_CreateSurface( 1280, 720, g_frc_osd_emul.merge_osd_addr[0] ); /* 720p  */
	if ( merge_surf_list[1] < 0 )	merge_surf_list[1]	= FBDEV_GFX_CreateSurface( 1280, 720, g_frc_osd_emul.merge_osd_addr[1] ); /* 720p  */
	...
	if ( g_frc_osd_emul.osd_view_flag & OSD_EMUL_VIEW_BACK_IMG )
	{
		...
		{
			full_hd_merge_surf = FBDEV_GFX_CreateSurface( 1920,1080, 0x0 /*NULL*/);
			FBDEV_CHECK_CODE( full_hd_merge_surf < 0, goto func_exit, "can't alloc HD surface. critical error !!\n");
		}
		FBDEV_GFX_GetSurfaceInfo ( full_hd_merge_surf, &surface_info );
	...
	else
	{
		if ( full_hd_merge_surf >= 0 )
		{
			FBDEV_GFX_FreeSurface( full_hd_merge_surf );
	...
	posd1_surf	= FBDEV_GFX_CreateSurfaceEx( 1280, 720, posd1_canvas->uiBitmapDataYPTR, posd1_canvas->stride );
	posd3_surf	= FBDEV_GFX_CreateSurfaceEx(  128, 128, posd3_canvas->uiBitmapDataYPTR, posd3_canvas->stride );
	...
	if ( g_frc_osd_emul.osd_view_flag & OSD_EMUL_VIEW_CRSR )
	{
		...
		if ( csr_x_pos < 1280 && csr_y_pos < 720 )
		{
			...
			FBDEV_GFX_StretchSurface( posd3_surf, crsr_surf, 0, 0, 128, 128, 0, 0, 88, 88 );	
		 ...
		 g_frc_osd_emul.osd_view_flag == (OSD_EMUL_VIEW_POSD1|OSD_EMUL_VIEW_CRSR) )
	{
		...
		if ( dst_surf >= 0 )
		{
			...
			{
				FBDEV_GFX_BlitSurface( crsr_bkup_surf, -1, dst_surf, 0, 0, crsr_bkup_rect.w, crsr_bkup_rect.h, crsr_bkup_rect.x, crsr_bkup_rect.y, FALSE );
			...
			FBDEV_GFX_BlitSurface( dst_surf, -1, crsr_bkup_surf, dx, dy, cw, ch, 0, 0, FALSE );
			...
			FBDEV_GFX_BlitSurface( crsr_surf, dst_surf, dst_surf, cx, cy, cw, ch, dx, dy, TRUE );
	...
	case (OSD_EMUL_VIEW_POSD0|OSD_EMUL_VIEW_POSD1):
	{
		...
		{
			FBDEV_GFX_StretchSurface( posd1_surf, merge_surf, 0, 0, posd1_canvas->input_win.w, posd1_canvas->input_win.h, 0, 0, 1280, 720 );
			FBDEV_GFX_BlitSurface( posd0_surf, merge_surf, merge_surf, 0, 0, 1280, 720, 0, 0, TRUE );
		}
		else
		{
			FBDEV_GFX_BlitSurface( posd0_surf, posd1_surf, merge_surf, 0, 0, 1280, 720, 0, 0, TRUE );
		}
	}
	break;
	...
	case OSD_EMUL_VIEW_POSD0:
	{
		...
		FBDEV_GFX_BlitSurface( posd0_surf, -1, merge_surf, 0, 0, 1280, 720, 0, 0, FALSE );
	}
	break;
	...
	case OSD_EMUL_VIEW_POSD1:
	{
		...
		{
			FBDEV_GFX_BlitSurface( posd1_surf, -1, merge_surf, 0, 0, 1280, 720, 0, 0, FALSE );
		}
	}
	break;
		...
		default:
		{
			...
			FBDEV_GFX_ClearSurface( merge_surf, 1280, 720 );
		}
		break;
	}
	...
	if ( g_frc_osd_emul.osd_view_flag & OSD_EMUL_VIEW_CRSR )
	{
		...
		FBDEV_GFX_BlitSurface( crsr_surf, merge_surf, merge_surf, cx, cy, cw, ch, dx, dy, TRUE );
	}

	/* speical code when back image is shown */
	if( g_frc_osd_emul.osd_view_flag & OSD_EMUL_VIEW_BACK_IMG )
	{
		int temp_surf = FBDEV_GFX_CreateSurface( 1920, 1080, 0x0 );
		int img_surf  = FBDEV_GFX_CreateSurface( 1920, 1080, posd0_canvas->back_image_mem.addr );
		PRINTK("^g^[fb] temp_surf %d, img_surf %d\n", temp_surf, img_surf );

		/* resize the merged POSD to be full HD size */ 
		FBDEV_GFX_StretchSurface( merge_surf, temp_surf, 0, 0, 1280, 720, 0, 0, 1920, 1080 );	
//		FBDEV_GFX_BlitSurface( merge_surf, -1, temp_surf, 0, 0, 1280, 720, 0, 0, FALSE );
		/* merge the resized POSD and image and generates one single full HD screen */
		FBDEV_GFX_BlitSurface( temp_surf, img_surf, full_hd_merge_surf, 0, 0, 1920, 1080, 0, 0, TRUE );

		FBDEV_GFX_FreeSurface ( img_surf );
		FBDEV_GFX_FreeSurface ( temp_surf );
	}
...
header_update:
	...
	if ( g_frc_osd_emul.osd_view_flag & OSD_EMUL_VIEW_BACK_IMG )
	{
		...
		memcpy ( (void*)merge_canvas->uiOSDHeaderPTR, (void*)&osd_hdr, sizeof(FBDEV_OSD_HDR_T) );
	}
...
static void FBDEV_L9B0_MergePOSD_1080p( int layer, BOOLEAN b_flip )
{
	...
	if ( crsr_bkup_surf < 0 )			crsr_bkup_surf		= FBDEV_GFX_CreateSurface( 128,  128, g_frc_osd_emul.crsr_bkup_addr );	 /* 128x128  */
	if ( merge_surf_list[0] < 0 )		merge_surf_list[0]	= FBDEV_GFX_CreateSurface( 1280, 720, g_frc_osd_emul.merge_osd_addr[0] ); /* 1280x720 */
	if ( merge_surf_list[1] < 0 )		merge_surf_list[1]	= FBDEV_GFX_CreateSurface( 1280, 720, g_frc_osd_emul.merge_osd_addr[1] ); /* 1280x720 */
	...
	/* create temporary surface to merge the POSD source */
	posd0_surf	= FBDEV_GFX_CreateSurface( 1280, 720, posd0_canvas->uiBitmapDataYPTR );
	posd1_surf	= FBDEV_GFX_CreateSurface( 1280, 720, posd1_canvas->uiBitmapDataYPTR );
	posd3_surf	= FBDEV_GFX_CreateSurface(  128, 128, posd3_canvas->uiBitmapDataYPTR );
...
posd_merge_done:
	{
		...
		FBDEV_GFX_StretchSurface( src_surf, merge_hd_surf, 0, 0, 1280, 720, 0, 0, 1920, 1080 );	
...
posd_stretch_done:
		...
		{
			int img_surf = FBDEV_GFX_CreateSurface( 1920, 1080, posd0_canvas->back_image_mem.addr );
			FBDEV_GFX_BlitSurface( merge_hd_surf, img_surf, merge_hd_surf, 0, 0, 1920, 1080, 0, 0, TRUE );
			FBDEV_GFX_FreeSurface ( img_surf );
		}
		...
		{
			PRINTK("[fb] draw cursor\n");
			FBDEV_GFX_BlitSurface( crsr_surf, merge_hd_surf, merge_hd_surf, cx, cy, cw, ch, dx, dy, TRUE );
		}
