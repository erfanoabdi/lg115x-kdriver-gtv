...
static int	_HDMI_ReadProcFunction(	UINT32 procId, char* buffer )
{
	...
	switch( procId )
	{
		...
		case PROC_ID_AVI:
		{
			LX_HDMI_AVI_PACKET_T packet;
			ret = HDMI_GetAviPacket(&packet);
			if (ret == 0)
			{
				len = 0;
				len += sprintf( buffer +len, "Pixel Encoding %d\n",packet.eAviPacket.ePixelEncoding);
				len += sprintf( buffer +len, "Active Info %d\n", packet.eAviPacket.eActiveInfo);
				len += sprintf( buffer +len, "Bar packet %d\n",  packet.eAviPacket.eBarInfo);
				len += sprintf( buffer +len, "Scan Info %d\n", packet.eAviPacket.eScanInfo);
				len += sprintf( buffer +len, "Color imetry %d\n",  packet.eAviPacket.eColorimetry);
				len += sprintf( buffer +len, "Picture Aspect Ratio %d\n",packet.eAviPacket.ePictureAspectRatio);
				len += sprintf( buffer +len, "Active Format AspectRatio %d\n", packet.eAviPacket.eActiveFormatAspectRatio);
				len += sprintf( buffer +len, "IT Content %d\n", packet.eAviPacket.eITContent);
				len += sprintf( buffer +len, "Extended Colorimetry %d\n",packet.eAviPacket.eExtendedColorimetry);
				len += sprintf( buffer +len, "RGB Quantization Range %d\n", packet.eAviPacket.eRGBQuantizationRange);
				len += sprintf( buffer +len, "Scaling %d\n",  packet.eAviPacket.eScaling);
				len += sprintf( buffer +len, "VideoIdCode 0x%x\n", packet.eAviPacket.VideoIdCode);
				len += sprintf( buffer +len, "YCC Quantizationi Range %d\n",  packet.eAviPacket.eYCCQuantizationRange);
				len += sprintf( buffer +len, "Content Type %d\n",packet.eAviPacket.eContentType);
				len += sprintf( buffer +len, "PixelRepeat %d\n", packet.eAviPacket.PixelRepeat);
				len += sprintf( buffer +len, "Top Bar End Line Number 0x%x\n", packet.eAviPacket.TopBarEndLineNumber);
				len += sprintf( buffer +len, "Bottom Bar Start Line Number 0x%x\n",  packet.eAviPacket.BottomBarStartLineNumber);
				len += sprintf( buffer +len, "Left Bar End Pixel Number 0x%x\n", packet.eAviPacket.LeftBarEndPixelNumber);
				len += sprintf( buffer +len, "Left Bar End Pixel Number 0x%x\n",  packet.eAviPacket.LeftBarEndPixelNumber);
				len += sprintf( buffer +len, "Right Bar End Pixel Number 0x%x\n",packet.eAviPacket.RightBarEndPixelNumber);
