...
static UINT32 _gHdmiAudioMute = 0;
static UINT32 _gHdmiPortStableCount = 0;
...
static LX_HDMI_SPD_PACKET_T _gPrevSPDPacket = {0, };
...
static int _HDMI_L9Bx_Set3DFullTimingInfo(LX_HDMI_TIMING_INFO_T *info);
static void	_HDMI_L9Bx_ClearResetLink(BOOLEAN reset);
static int _HDMI_L9Bx_SetInternalMute(LX_HDMI_MUTE_CTRL_T *interMute);
 ...
 *				Local function
 *****************************************************************************/
static void	_HDMI_L9Bx_ClearSWResetAll(void)
{
...
{
	ULONG	flags = 0;
	LX_HDMI_MUTE_CTRL_T 		muteCtrl = {0, 0, 0, 3}; // All Clear
	...
	if( _gSCDT == 1)
	{
		if(_gPortConCount == 0)
		{
			_HDMI_L9Bx_SetInternalMute(&muteCtrl);	// All Clear
	...
	else
	{
		...
		_gHdmiPortStableCount = 0;		//Clear HDMI Port Stable Count(If port is changed, this interrupt is occurred.
	...
	HDMI_PRINT("HDMI_L9Bx_GetPortConnection [%d]\n",_gSCDT);
	return _gSCDT;
}
...
int HDMI_L9Bx_GetTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	LX_HDMI_MUTE_CTRL_T 		muteCtrl = {0, 0, 0, 3};
	BOOLEAN mode = 0;
	...
	if(!HDMI_L9Bx_GetPortConnection())
	{
	...
	if(_gIntrAVICscState == HDMI_AVI_CSC_CHANGE_STATE || _gIntrAVICscState < HDMI_AVI_CSC_TEMP_STATE)
	{
		if(_gIntrAVICscState != HDMI_AVI_CSC_CHANGE_STATE)
		{
			...
			_HDMI_L9Bx_SetInternalMute(&muteCtrl);
		}
		_gIntrAVICscState++;
...
#endif
			HDMI_L9Bx_SetPixelRepetition(); 		// for specific 13.5MHz input(480i, 576i) L9 A0 only, L9 B0 don't use
		}
...
func_read :
	...
	if(_gTimingChanged > 0 && _gIntrVideoState >= HDMI_VIDEO_INIT_STATE)
	{
	...
	HDMI_L9Bx_SetPixelRepetition();			// for specific 13.5MHz input(480i, 576i) L9 A0 only, L9 B0 don't use
	HDMI_L9Bx_SetAutoSyncPolarity();			// sync inversion for UD and 3D(SS_Full) format
	...
	_HDMI_L9Bx_Set3DFullTimingInfo(&bufTiming);

	if( (bufTiming.vActive == 0) && (bufTiming.hActive == 0) )		goto func_exit;

	if(bufTiming.full_3d_timing == LX_HDMI_NORMAL_TIMING)		///< Master DVI to HDMI Cable #20, #21 h and v Active info +/-1 error
	{
...
//////////////* Horizontal & Vertical frequency //////////////////////////
	...
	if(tmdsClock != 0)		// XXX.XX KHz
	{
		...
		if(bufTiming.full_3d_timing == LX_HDMI_3D_FIELD_ALTERNATIVE)
		{
...
func_exit2:
	...
	info->full_3d_timing	= _gPrevTiming.full_3d_timing; 	///< Full 3D Timing
	return 0;
}
...
*/
	...
	//A0
	tmp8 = glink_119_bx.reg_pkt_avi_dat_pip0_1;
	packet->eAviPacket.eActiveInfo = _gPrevAVIPacket.eAviPacket.eActiveInfo = (tmp8& 0x40)>>4;
...
#ifdef VIDEO_DEBUG
	HDMI_DEBUG(" HDMI_L9Bx_GetSpdPacket  Read \n");
#endif
	return 0;
...
int HDMI_L9Bx_SetPort(LX_HDMI_HPD_T *hpd)
{
	...
	_HDMI_L9Bx_ClearResetLink(1);

	/* Port	Power down control */
	HDMI_L9Bx_SetPowerControl(HDMI_PHY0_PORT , 0);

	// try to do power on without status check
	HDMI_L9Bx_SetPowerControl(HDMI_PHY0_PORT , 1);

	//OS_MsecSleep(50);

	// All Link Reset Clear
	_HDMI_L9Bx_ClearResetLink(0);
...
int HDMI_L9Bx_SetPowerControl(HDMI_PHY_TYPE_T type , int power)
{
	...
	HDMI_PRINT("HDMI_L9Bx_SetPowerControl [%s] \n", power?"Power Up":"Power Down");
...
int HDMI_L9Bx_RunReset(void)
{
	...
	//2013-02-20 sh.myoung
	// UD Format(2.97Gbps) 에 대한 Sampling clock jitter 문제 개선.
	VPORT_I2C_Read((UINT32*)&ghdmi_phy_n_13_bx);
	ghdmi_phy_n_13_bx.cr_kvco_offset = 2;
	VPORT_I2C_Write((UINT32*)&ghdmi_phy_n_13_bx);
...
*/
	...
	else if(eVideoFormat == LX_FORMAT_3D_FORMAT)
	{
		...
		switch(e3DStructure)
		{
			...
			default:
				 bufTiming.full_3d_timing = e3DStructure;
				goto func_exit;
				break;
		}
	}
 ...
 *  @return int
*/
static int _HDMI_L9Bx_GetAudioTypeAndFreq(LX_HDMI_AUDIO_TYPE_T *audioType, LX_HDMI_SAMPLING_FREQ_T *samplingFreq)
{
	int ret = 0;
	...
	if(glink_387_bx.reg_achst_byte0_pip0 & HDMI_AUDIO_SAMPLE_PCM_MASK)	//bit 1, 0 : PCM, 1 : non-PCM
	{
 ...
 *  @return int
*/
static int _HDMI_L9Bx_GetAudioFreqFromTmdsClock(LX_HDMI_SAMPLING_FREQ_T *samplingFreqFromTmds)
{
	int ret = 0;
	...
	AUDIO_DEBUG("Get TDMS Clock : ui64TmdsClock = %llu, ui8TmdsClockHigh = 0x%X, ui8TmdsClockLow = %d\n", ui64TmdsClock, ui8TmdsClockHigh, ui8TmdsClockLow);
...
static int _HDMI_L9Bx_DebugAudioInfo(void)
{
	int ret = 0;
...
int HDMI_L9Bx_GetAudioInfo(LX_HDMI_AUDIO_INFO_T *audio_info)
{
	...
	if(intrAudioState == HDMI_AUDIO_STABLE_STATE && _gIntrVideoState == HDMI_VIDEO_STABLE_STATE)
	{
		...
		if((_gHdmiAudioPrintCount % HDMI_AUDIO_RECHECK_TIME) == 0)	// 500 mili-seconds, DDI calls every 50 ms
		{
			//Get HDMI Audio Type and Sampling Frequency
			(void)_HDMI_L9Bx_GetAudioTypeAndFreq(&audioType, &samplingFreq);

			//Get a sampling frequency from TMDS clock
			(void)_HDMI_L9Bx_GetAudioFreqFromTmdsClock(&samplingFreqFromTmds);
			...
			{
				HDMI_DEBUG("Recheck 1 => HDMI_GetAudioInfo : type = %d(%d), tmds freq = %d(%d), freq = %d, state = %d, mute = %d\n",	\
							audioType, _gHdmiAudioInfo.audioType, samplingFreqFromTmds, _gHdmiAudioInfo.samplingFreq,	\
							samplingFreq, _gIntrAudioState, _gHdmiAudioMute);
		...
		//Clear HDMI Port Stable Count(If port is changed, this interrupt is occurred.
		...
		audio_info->bPortConnected = _gHdmiAudioInfo.bPortConnected;
		audio_info->bInterlaced    = _gHdmiAudioInfo.bInterlaced;

		//Debug print
		if((_gHdmiAudioPrintCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME) == 0)	//DDI calls every 50 ms
		{
			HDMI_PRINT("HDMI_GetAudioInfo 1 : type = %d, freq = %d, bCon = %d, bInt = %d, state = %d\n",	\
						audio_info->audioType, audio_info->samplingFreq, audio_info->bPortConnected, audio_info->bInterlaced, _gIntrAudioState);
		}
		  ...
		  && (_gTimingCnt >= HDMI_VIDEO_READ_STATE) )
	{
		...
		if((_gHdmiAudioPrintCount % HDMI_AUDIO_RECHECK_TIME) == 0)	// 500 mili-seconds, DDI calls every 50 ms
		{
			...
			if( (audioType 	  		  != _gHdmiAudioInfo.audioType)
			  ||(samplingFreqFromTmds != _gHdmiAudioInfo.samplingFreq) )
			{
				HDMI_DEBUG("Recheck 2 => HDMI_GetAudioInfo : type = %d(%d), tmds freq = %d(%d), freq = %d, state = %d, mute = %d\n",	\
							audioType, _gHdmiAudioInfo.audioType, samplingFreqFromTmds, _gHdmiAudioInfo.samplingFreq,	\
							samplingFreq, _gIntrAudioState, _gHdmiAudioMute);
	...
	if(gBootData->mode)
	{
		...
		if(audioType == LX_HDMI_AUDIO_PCM)
		{
			  ...
			  &&(samplingFreqFromTmds != LX_HDMI_SAMPLING_FREQ_44_1KHZ) )
			{
				...
				if(_gHdmiFreqErrorCount < HDMI_AUDIO_FREQ_ERROR_TIME)
				{
				...
				{
					HDMI_ERROR("HDMI_GetAudioInfo : Freq Error => Force 48KHz(status = %d, tmds = %d)!!!\n",	\
								samplingFreq, samplingFreqFromTmds);
				}
			}
			  ...
			  ||(samplingFreqFromTmds == LX_HDMI_SAMPLING_FREQ_768_KHZ))
			{
				...
				//Update HDMI audio mute status
				if(_gHdmiAudioMute == 0)
				{
					_gHdmiAudioMute = 1;

					if((_gHdmiAudioPrintCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME) == 0)	//DDI calls every 50 ms
						HDMI_DEBUG("[%d] HDMI_GetAudioInfo : HDMI Audio Mute On !!!\n", __L__);				
				}
			...
			else
			{
				...
				//Update HDMI audio mute status
				if(_gHdmiAudioMute == 1)
				{
					_gHdmiAudioMute = 0;

					if((_gHdmiAudioPrintCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME) == 0)	//DDI calls every 50 ms
						HDMI_DEBUG("[%d] HDMI_GetAudioInfo : HDMI Audio Mute Off !!!\n", __L__);				
				}
		...
		VPORT_I2C_Read((UINT32*)&glink_363_bx);
		bInterlaced = glink_363_bx.reg_intrl_pip0;
		  ...
		  ||(bPortConnected != _gHdmiAudioInfo.bPortConnected) || (bInterlaced	!= _gHdmiAudioInfo.bInterlaced) )
		{
			...
			HDMI_DEBUG("HDMI_GetAudioInfo : type = %d(0x%X), freq = %d, bCon. = %d, bInt. = %d, mute = %d, PSC = %d\n",	\
						audioType, glink_394_bx.reg_burst_pc_pip0_0 & 0x1F, samplingFreq, bPortConnected, bInterlaced, _gHdmiAudioMute, _gHdmiPortStableCount);

#ifdef	ENABLE_HDMI_AUDIO_DEBUG
			//Print Audio Debug Info.
			(void)_HDMI_L9Bx_DebugAudioInfo();
#endif		//#ifdef	ENABLE_HDMI_AUDIO_DEBUG
		}
	}
	else
	{
		...
		if(audioType != _gHdmiAudioInfo.audioType)
		{
			HDMI_DEBUG("HDMI_GetAudioInfo : type = 0x%X, freq = 0x%X, mute = %d, PSC = %d\n", audioType, samplingFreq, _gHdmiAudioMute, _gHdmiPortStableCount);
		}
	}
...
func_exit:
	...
	HDMI_PRINT("HDMI_GetAudioInfo : type = %d, freq = %d, bCon = %d, bInt = %d, state = %d\n",	\
				audio_info->audioType, audio_info->samplingFreq, audio_info->bPortConnected, audio_info->bInterlaced, _gIntrAudioState);

	return ret;
}

int _HDMI_L9Bx_SetInternalMute(LX_HDMI_MUTE_CTRL_T *interMute)
{
...
int HDMI_L9Bx_SetArc(LX_HDMI_ARC_CTRL_T *arc)
{
	...
	ghdmi_phy_n_26_bx.odt_sel = arc->bARCMute * 2; // Off : 0, On : 2(default)
	...
	//Enable ARC in Phy0
	glink_0_bx.reg_arc_src_prt0 = arc->path;

	VPORT_I2C_Read((UINT32*)&ghpd_out_control_1_bx);
	ghpd_out_control_1_bx.phy0_phy_arc_pdb = arc->bARCMute;
	VPORT_I2C_Write((UINT32*)&ghpd_out_control_1_bx);

	VPORT_I2C_Write((UINT32*)&glink_0_bx);

	HDMI_PRINT("HDMI_L9Bx_SetArc [%s] Arc Path = %d,  \n", arc->bARCMute?"On":"Off", arc->path);
#endif
	return ret;
}
...
{
	VPORT_I2C_Read((UINT32*)&ghdmi_phy_n_01_bx);
	ghdmi_phy_n_01_bx.resetb_all = power->enable;
	ghdmi_phy_n_01_bx.pdb_all = power->enable;
	VPORT_I2C_Write((UINT32*)&ghdmi_phy_n_01_bx);

	HDMI_ERROR(" HDMI_L9Bx_PowerConsumption Power[%s]\n", (power->enable ? "On" : "Off"));
	return 0;
}
...
int HDMI_L9Bx_SetUDn3DControl(LX_HDMI_UD_3D_MODE_T *udn3d)
{
	...
	HDMI_PRINT("HDMI_L9Bx_SetUDn3DControl[0:2D, 1:UD 2:SS 3:FP] ==>[ %d ]\n", *udn3d);
...
{
	ULONG	flags = 0;
	LX_HDMI_MUTE_CTRL_T 		muteCtrl = {0, 0, 0, 1};
	...
	if(gl9a_hdmi0_intr_6_bx.intr_vf_stable)
	{
		...
		if((_gTimingChanged == 0) && (_gTimingCnt >= gBootData->timingCn[gBootData->hdmi_switch]))
		{
			...
			_gTimingCnt = HDMI_VIDEO_READ_STATE;
			HDMI_DEBUG("[HDMI_VF_STABLE]  HDMI Info changed at other external input(PSC = %d)\n", _gHdmiPortStableCount);

			//check a port stable count
			if(_gHdmiPortStableCount >= HDMI_AUDIO_PORT_STABLE_COUNT)
			{
				//spin lock for protection for audio
				spin_lock_irqsave(&_gIntrHdmiAudioLock, flags);
				_gIntrAudioState = HDMI_AUDIO_STABLE_STATE;
...
void HDMI_L9Bx_EnableInterrupt(void)
{
	...
	if (request_irq(L9_IRQ_HDMI1, HDMI_IRQHandler, 0, "hdmi", NULL))
	{
		HDMI_ERROR("request_irq  is failed\n");
	}

	HDMI_ERROR("HDMI_L9Bx_EnableInterrupt\n");
