...
//	OS_DEBUG_EnableModuleByIndex (g_aad_debug_fd, 4, DBG_COLOR_NONE);	//AAD_DEBUG
	...
	switch (lx_chip_rev())
	{
		...
		case LX_CHIP_REV(L8, B0):
		case LX_CHIP_REV(L8, A1):
			AAD_DEBUG_TMP ("LX_CHIP_REV(L8, XX)(0x%X)\n", lx_chip_rev());
...
AAD_Close(struct inode *inode, struct file *file)
{
	...
	if ( my_dev->dev_open_count <= 0 )
	{
		...
		if ( g_pstAADMonitorTask != NULL )
		{
			...
			AAD_PRINT("kthread_stop AAD Monitor task\n");
		}
	}
	/* END */
	...
	switch(cmd)
	{
		case AAD_IOW_INIT_MODULE:
		{
			...
			if ( my_dev->is_initialized == 0 )
			{
				...
				ret = AAD_SIF_InitModule( i_eInitType );

				//set my_dev->is_initialized value
				if(ret == RET_OK)
					my_dev->is_initialized = 1;

				//Create thread for AAD module for KOREA A2 Signal
				if ( i_eInitType == LX_AAD_INIT_ATSC_A2_SELECT )
				{
		...
		{
			LX_AAD_HIGH_DEV_LEVEL_T 	i_eHighDevLevel;

			memset(&i_eHighDevLevel, 0x0, sizeof(LX_AAD_HIGH_DEV_LEVEL_T));

			AAD_TRACE("AAD_IOW_SIF_SET_HIGH_DEV_MODE...\n");

			if ( copy_from_user( &i_eHighDevLevel, (void __user *)arg, sizeof(LX_AAD_HIGH_DEV_LEVEL_T ) ) )
				return RET_ERROR;
