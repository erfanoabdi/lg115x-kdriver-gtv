...
const static	LX_GFX_CFG_T 		g_gfx_cfg_l9 =
{
    .b_hw_scaler 			= TRUE,
...
int GFX_L9_InitHW(void)
{
	...
	GFX_CHECK_CODE( g_gfx_reg_l9a0 == NULL, return RET_ERROR, "can't alloc memory for regs\n");

	g_gfx_reg_l9a0_cache = (GFX_L9A0_REG_T *)OS_Malloc( gpGfxRegCfg->reg_size );
	GFX_CHECK_CODE( g_gfx_reg_l9a0_cache == NULL, return RET_ERROR, "out of memory\n");
	...
	GFX_CHECK_CODE( request_irq( gpGfxRegCfg->irq_num, (irq_handler_t)GFX_L9_ISRHandler, 0, "gfx_irq", NULL), /* nop */, "request irq failed\n" );
...
//	g_gfx_reg_l9a0->scaler_bypass.scaler_bypass = 1;
	...
	/* make default scaler filter value */
	g_gfx_scaler_filter.phase_offset	= 0x0;
	g_gfx_scaler_filter.boundary_mode	= 0x1;
	g_gfx_scaler_filter.sampling_mode	= 0x0;
	g_gfx_scaler_filter.numerator		= 0x0;
	g_gfx_scaler_filter.denomidator		= 0x0;
	g_gfx_scaler_filter.filter_bank		= 0x0;
