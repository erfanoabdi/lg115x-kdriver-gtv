...
void GFX_L9_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler)
{
	switch( scaler->cmd )
	{
		...
		case GFX_SCALER_CMD_START:
		{
			...
			GFX_Wr01( scaler_phase_offset, 	scaler_phase_offset, 	scaler->filter_data.phase_offset );
			GFX_Wr01( scaler_boundary_mode, scaler_boundary_mode, 	scaler->filter_data.boundary_mode );
			GFX_Wr01( scaler_sampling_mode, scaler_sampling_mode, 	scaler->filter_data.sampling_mode );
			GFX_Wr01( scaler_numerator,   	scaler_numerator, 		scaler->filter_data.numerator );
			GFX_Wr01( scaler_denominator, 	scaler_denominator, 	scaler->filter_data.denomidator);
			GFX_Wr01( scaler_hcoef_index, 	scaler_hcoef_index, 	scaler->filter_data.filter_bank);
			GFX_Wr01( scaler_vcoef_index, 	scaler_vcoef_index, 	scaler->filter_data.filter_bank);

			if ( scaler->filter == GFX_SCALER_FILTER_BILINEAR )
			{
			...
			else	/* custom filter */
			{
				...
				/* TODO : override some default variable for custom filter mode */
			}
