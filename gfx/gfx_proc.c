...
static OS_PROC_DESC_TABLE_T	_g_gfx_device_proc_table[] =
{
	...
	{ "scalerp",	PROC_ID_SCALER_PARAM, OS_PROC_FLAG_READ|OS_PROC_FLAG_WRITE },
...
static int	_GFX_ReadProcFunction(	UINT32 procId, char* buffer )
{
	...
	switch( procId )
	{
		...
		case PROC_ID_SURFACE:
		{
			...
			{
				GFX_SURFACE_OBJ_T*	surface = &g_stSurface[i];
...
#endif
					printk( "[%3x(%3d)] %d:%3d:%4d:%08x - %d:%2x:%4dx%4d - %d:%4d - %d - %8p:%8p:%8x:%d\n",
						i, i,
						...
						(surface->pal)? 1:0,	
						surface->surf.phys_addr, surface->mem.phys_addr, surface->mem.offset, surface->mem.length );
				}
			}
		...
		case PROC_ID_SCALER_PARAM:
		{
			ret = sprintf( buffer, "phase:%x\nboundary:%x\nsampling:%x\nnumerator:%x\ndenomindator:%x\nbank:%x\n", 
								g_gfx_scaler_filter.phase_offset,
								g_gfx_scaler_filter.boundary_mode,
								g_gfx_scaler_filter.sampling_mode,
								g_gfx_scaler_filter.numerator,
								g_gfx_scaler_filter.denomidator,
								g_gfx_scaler_filter.filter_bank );
		}
		break;
