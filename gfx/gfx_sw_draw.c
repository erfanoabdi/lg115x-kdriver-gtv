...
int GFX_RunSwScale(LX_GFX_SW_SCALE_CTRL_PARAM_T* param)
{
    ...
    GFX_SURFACE_OBJ_T*   src0_surface = &(g_stSurface[param->src0.surface_fd]);
    GFX_SURFACE_OBJ_T*   dst_surface  = &(g_stSurface[param->dst.surface_fd]);
...
MARK_TICK(tk0);
	...
	GFX_CHECK_CODE( ret != RET_OK, goto func_exit, "can't ioremap src port\n");
	...
	GFX_CHECK_CODE( ret != RET_OK, goto func_exit, "can't ioremap dst port\n");
...
MARK_TICK(tk1);
    ...
    switch( param->scaler_mode & 0xff00 )
    {
        ...
        case LX_GFX_SW_SCALER_MODE_BILINEAR:
        {
            ...
            GFX_CHECK_CODE( sub_algo > NELEMENTS(g_bilinear_func_list), goto func_exit, "unkown bilinear 0x%x\n", param->scaler_mode );
...
int GFX_RunSwBlit(LX_GFX_SW_BLIT_CTRL_PARAM_T* param)
{
    ...
    GFX_SURFACE_OBJ_T*   src_surface = &(g_stSurface[param->src0.surface_fd]);
    GFX_SURFACE_OBJ_T*   dst_surface = &(g_stSurface[param->dst.surface_fd]);
