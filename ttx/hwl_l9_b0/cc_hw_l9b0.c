...
----------------------------------------------------------------------------------------*/
#define DEBUG_PRINT_INCOMING_CC_PACKET 1
#define DEBUG_PRINT_IF_NOT_PACKET_9_BYTE 0
...
int CC_L9B0_SetInputFormat(eVBIDataType type)
{
	...
	switch(type)
	{
		...
		case eClosedCaptionEURO:
			...
			VPORT_L9B0_Wr(vbi_020, 0x5A548282);    // 2011.12.10 won.hur : Changed CC find start h-sync value
...
int CC_L9B0_ParsingVBIData(unsigned int *packet_array)
{
	...
	{
		packet_count++;
#if DEBUG_PRINT_IF_NOT_PACKET_9_BYTE
		CC_PRINT("CC_MESSAGE_B= PACKET CANT BE DIVIDED BY 9!\n");
#endif
	}
	if(packet_count <= 5)
	{
		...
		{
			if( (start < gstRegCfgTTX->vbi_register_size) && (end < gstRegCfgTTX->vbi_register_size) /*&& (count <= 4*5)*/ )
			{
				valid_data_flag = 1;
				if( ((gstRegCfgTTX->vbi_register_size>>2)*4 - start_inc) < 9)
				{

					first_bytes = ((gstRegCfgTTX->vbi_register_size>>2)*4 - start_inc);
					...
					{
						CC_PRINT("CC_MESSAGE_A= %4d %4d %4d", start, end, count);
