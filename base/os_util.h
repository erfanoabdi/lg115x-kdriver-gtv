...
#define OS_LockMutexEx(pSem,timeout)	OS_LockMutex_Tag(pSem,timeout, __FILE__, __FUNCTION__, __LINE__)

#define OS_LockMutex(pSem)				OS_LockMutex_Tag(pSem,OS_INFINITE_WAIT, __FILE__, __FUNCTION__, __LINE__)
 ...
 * @return NONE
 */
#define OS_UnlockMutex(pSem)		OS_UnlockMutex_Tag(pSem, __FILE__, __FUNCTION__, __LINE__)
 ...
 * receive event.
*/
#define OS_RecvEvent(pEvnt,ev,rEv,option,timeout)	OS_RecvEvent_Tag( pEvnt, ev, rEv, option, timeout, __FILE__, __FUNCTION__, __LINE__)
/**
 * @def OS_SendEvent
 * send event.
*/
#define	OS_SendEvent(pEvnt,ev)		OS_SendEvent_Tag( pEvnt, ev, __FILE__, __FUNCTION__, __LINE__)

/**
 * @def OS_ClearEvent
 * clear event.
*/
#define OS_ClearEvent(pEvnt)		OS_ClearEvent_Tag( pEvnt, __FILE__, __FUNCTION__, __LINE__)
/** @} */
...
*/
#define	OS_MsecSleep(msec)						OS_MsecSleep_Tag(msec, __FILE__, __FUNCTION__, __LINE__)
#define OS_NsecDelay(nsec)						OS_NsecDelay_Tag(nsec, __FILE__, __FUNCTION__, __LINE__)
#define OS_UsecDelay(usec)						OS_UsecDelay_Tag(usec, __FILE__, __FUNCTION__, __LINE__)
...
/**
 ...
 @param pTimer [IN] pointer to OS_TIMER_T
*/
#define OS_InitTimer(pTimer)										OS_InitTimer_Tag(pTimer, __FILE__, __FUNCTION__, __LINE__)

#define OS_StartTimer(pTimer,timer_callback,option,timeout,data)	OS_StartTimer_Tag(pTimer,timer_callback,option,timeout,data, __FILE__, __FUNCTION__, __LINE__)

#define OS_StopTimer(pTimer)										OS_StopTimer_Tag(pTimer, __FILE__, __FUNCTION__, __LINE__)
 ...
 *	@param block_num  [IN]	toal number of block ( total memory size = block_num * block_size )
 *	@return RET_OK if success, RET_ERROR otherwise.
 *
...
#define OS_InitRegion(pRgn,phys_addr,size)	\
									OS_InitRegion_Tag(pRgn,phys_addr,size, __FILE__, __FUNCTION__, __LINE__)
#define OS_InitRegionEx(pRgn,phys_addr,block_size,block_num)	\
									OS_InitRegionEx_Tag(pRgn, (void*)phys_addr, block_size, block_num, __FILE__, __FUNCTION__, __LINE__ )
#define OS_CleanupRegion(pRgn)		OS_CleanupRegion_Tag(pRgn, __FILE__, __FUNCTION__, __LINE__)
#define OS_MallocRegion(pRgn,size)	OS_MallocRegion_Tag(pRgn,size, __FILE__, __FUNCTION__, __LINE__)
#define OS_FreeRegion(pRgn,ptr)		OS_FreeRegion_Tag(pRgn,ptr, __FILE__, __FUNCTION__, __LINE__)
#define OS_GetRegionInfo(pRgn,pMemInfo)	OS_GetRegionInfo_Tag(pRgn, pMemInfo, __FILE__, __FUNCTION__, __LINE__)
/** @} */
...
*/
#define OS_OpenFile(pFile,fname,flags,mode)		OS_OpenFile_Tag(pFile, fname, flags, mode, __FILE__, __FUNCTION__, __LINE__)
#define OS_CloseFile(pFile)						OS_CloseFile_Tag(pFile , __FILE__, __FUNCTION__, __LINE__)
#define OS_SeekFile(pFile, offset, whence)		OS_SeekFile_Tag(pFile, offset, whence, __FILE__, __FUNCTION__, __LINE__)
#define OS_SizeOfFile(pFile)					OS_SizeOfFile_Tag(pFile,  __FILE__, __FUNCTION__, __LINE__)
#define OS_WriteFile(pFile,buffer,count)		OS_WriteFile_Tag(pFile, buffer, count,  __FILE__, __FUNCTION__, __LINE__)
#define OS_ReadFile(pFile,buffer,count)			OS_ReadFile_Tag(pFile, buffer, count,  __FILE__, __FUNCTION__, __LINE__)
 ...
 *
 */
#define	OS_FAILURE()				OS_FAILURE_Tag(__FILE__, __FUNCTION__, __LINE__)
...
extern void*	OS_KMalloc_Tag			( size_t size,
											const char* szFile, const char* szFunc, const int nLine );
extern void		OS_KFree_Tag			( void* ptr,
											const char* szFile, const char* szFunc, const int nLine );

extern int		OS_InitRegion_Tag		( OS_RGN_T* pRgn, const void* ptr, const int size,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_InitRegionEx_Tag		( OS_RGN_T* pRgn, const void* phys_addr, const int block_size, const int block_num,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_CleanupRegion_Tag	( OS_RGN_T* pRgn,
											const char* szFile, const char* szFunc, const int nLine );
extern void*	OS_MallocRegion_Tag		( OS_RGN_T* pRgn, size_t size,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_FreeRegion_Tag		( OS_RGN_T* pRgn, void* ptr,
											const char* szFile, const char* szFunc, const int nLine );
extern int		OS_GetRegionInfo_Tag	( OS_RGN_T* pRgn, OS_RGN_INFO_T* pMemInfo,
											const char* szFile, const char* szFunc, const int nLine );
