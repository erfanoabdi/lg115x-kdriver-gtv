...
static int CI_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if ( gpstCIReg )
	{
		...
		CI_DBG_INFO("[%s] done suspend\n", CI_MODULE);
	}
	else
	{
		CI_DBG_INFO("[%s] suspend done - -\n", CI_MODULE);
	}
...
static int CI_resume(struct platform_device *pdev)
{
	...
	if ( gpstCIReg )
	{
		...
		CI_DBG_INFO("[%s] done resume\n", CI_MODULE);
	}
	else
	{
		CI_DBG_INFO("[%s] resume done - -\n", CI_MODULE);
	}
...
int CI_Init(void)
{
	...
	if ( g_ci_debug_fd >= 0 )
	{
		...
		OS_DEBUG_EnableModuleByIndex ( g_ci_debug_fd, 0, DBG_COLOR_NONE );
		OS_DEBUG_EnableModuleByIndex ( g_ci_debug_fd, 1, DBG_COLOR_NONE );
		OS_DEBUG_EnableModuleByIndex ( g_ci_debug_fd, 2, DBG_COLOR_NONE );
		OS_DEBUG_EnableModuleByIndex ( g_ci_debug_fd, 3, DBG_COLOR_NONE );
	}
	else
	{
	...
	else
	{
		...
		{
			CI_DBG_INFO("[%s] platform register done\n", CI_MODULE);
		}
	}
#endif
...
static int CI_Close(struct inode *inode, struct file *file)
{
	...
	CI_DBG_INFO("[ CI_Close: CI device closed  <<S>> ]  \n");
	...
	switch(cmd)
	{
		...
		case CI_IOR_READ_DATA:
		{
			if( arg )
			{
				...
				{
					CI_DBG_INFO("[ CI_Ioctl(CI_IOR_READ_DATA): copy_to_user <<F>> ]\n");
		...
		case CI_IOR_NEGOTIATE_BUF_SIZE:
		{
			...
			if( ret == 0 )
			{
				...
				CI_DBG_INFO ( ( "Negotiated BUF size %d <<S>> ]\n", uwBufSize ) );

				if( copy_to_user( ( void __user * )arg, &uwBufSize, sizeof( UINT32 ) ) )
	 	        {
					CI_DBG_INFO("[ CI_Ioctl(CI_IOR_NEGOTIATE_BUF_SIZE): copy_to_user <<F>> ] \n");
			...
			{
				CI_DBG_INFO("[ CI_Ioctl(CI_IOR_NEGOTIATE_BUF_SIZE): "
		...
		case CI_IOW_WRITE_DATA:
		{
			if( arg )
			{
				...
				if ( copy_from_user( &stReadData, (void __user *)arg, sizeof( LX_CI_IOCTL_PARAM_T ) ) )
				{
					CI_DBG_INFO("[ CI_Ioctl(CI_IOW_WRITE_DATA): copy_from_user <<F>> ]\n");
		...
		case CI_IOR_READ_IIR_STATUS:
		{
			...
			if( ret == 0 )
			{
				CI_DBG_INFO(("[ CI_Ioctl(CI_IOR_READ_IIR_STATUS): Read IIR status is %s <<S>> ]\n", ((uwIIRStatus)? ("RESET") : ("NO"))));

				if ( copy_to_user( ( void __user * )arg, &uwIIRStatus, sizeof( UINT32 ) ) )
	 	        {
					CI_DBG_INFO("[ CI_Ioctl(CI_IOR_READ_IIR_STATUS): copy_to_user <<F>> ]\n");
		...
		case CI_IOR_CHECK_CAPABILITY:
		{
			...
			if( ret == 0 )
			{
				if ( copy_to_user( ( void __user * )arg, &stData, sizeof( LX_CI_IOCTL_PARAM_CAMTYPE ) ) )
	 	        {
					CI_DBG_INFO("[ CI_Ioctl(CI_IOR_CHECK_CAPABILITY): copy_to_user <<F>> ]\n");
			...
			{
				CI_DBG_INFO("[ CI_Ioctl(CI_IOR_CHECK_CAPABILITY): Check CAM Type <<F>> ]\n");
			}
		}
		break;
		...
		case CI_IO_PRINT_REG:
		{
			ret = CI_RegPrint();
		}
		break;

		case CI_IO_CAM_POWEROFF:
		{
			ret = CI_CAMPowerOff( );
		}
		break;

		case CI_IO_CAM_POWERONCOMPLETED:
		{
