...
#define MAX_VALID_TUPLE_CNT_CHK	26
...
SINT32 CIS_Config(UINT8 mId)
{
	...
	SINT16			nSleepTime = SLEEP_VALUE_INIT;


	if( mId != MOD_A )
		return CIS_FAIL;

	CIS_INFO( ( "Configuring CIS of module %c \n", 'A'+mId ) );

	pStatus = &CIS_infoTupleStatus[mId];

	CIS_INFO( ( "baseAddr: 0x%x \n", baseAddr ) );
	...
	if( chkCnt <= 0 )
	{
		CIS_ERR( ( "CIS_Config: the first tuple is invalid " ) );
		goto CIS_CONFIG_FAIL;
	}
...
CIS_READ_ANALYZE_STEP:
	...
	while( chkCnt-- )
	{
		...
		CIS_TUPLE_DUMP( ( "Tuple Code=0x%02X, Tuple Length=%3d -------------------------------\n",
																		aui8Buf[0], aui8Buf[1]+2 ) );
		CIS_TUPLE_DUMP( ( "-----------------------------------------------------------------\n" ) );

		if( CIS_ParseTuple(mId, aui8Buf, pStatus) == CIS_PARSETIMEOUT )
		{
			CIS_ERR( ( "CIS configuration FAILED #1\n" ) );
			break;
		}
		if( rdRet == TUPLE_LAST )
			break;
	...
	{	// if the abnormal case, the try again (max 3, delay time is changed)
		...
		{
			CIS_ERR( ( "Again get CIS configuration (%d try)\n", nNumofTry_ReadTuple ) );		
		...
		{
			CIS_ERR( ( "CIS configuration FAILED #2\n" ) );		
		}
	}
...
SINT32 CIS_WriteCOR(UINT8 mId)
{
	...
	if( lx_chip_rev() >= LX_CHIP_REV(L9, A0) )
	{
		...
		//wl.gui current kernel(3.4.5+) not auto remap
...
static int CIS_ParseTuple(MOD_ID mId, UINT8 *pBuf, CISTPL_STATUS *pStatus)
{
	...
	default :
		...
		if(CIS_ParseTuple_count >= MAX_TUPLE_COUNT_ERR)
	   	{
		     CIS_ERR( ( "^e^========@ CIS_ParseTuple error =======\n" ) );
...
static TPL_STATUS CIS_ParseVers1(MOD_ID	mId, UINT8 *pBuf)
{
	...
	{
		CIS_PARSE_DUMP( ( "CIS - Actual Tuple Len - %d, Source buf(Tuple) size - %d\n",
	...
	CIS_PARSE_DUMP( ( "  Major Version: 0x%02X\n", pVers1->major ) );
	CIS_PARSE_DUMP( ( "  Minor Version: 0x%02X\n", pVers1->minor ) );
	...
	CIS_PARSE_DUMP( ( "  Manufact Str : %s\n", pVers1->manufact ) );
	CIS_PARSE_DUMP( ( "  Product Str  : %s\n", pVers1->product ) );
	CIS_PARSE_DUMP( ( "  Add1 Str	  : %s\n", pVers1->add1 ) );
	CIS_PARSE_DUMP( ( "  Add2 Str	  : %s\n", pVers1->add2 ) );
	CIS_PARSE_DUMP( ( "  Is it CI+ CAM: %d\n", pVers1->ui8IsItCIPlus ) );
...
static TPL_STATUS CIS_ParseConfig(MOD_ID mId, UINT8 *pBuf)
{
	...
	CIS_PARSE_DUMP( ( "  TPCC_RASZ : %02d bytes\n", tpccRasz ) );
	CIS_PARSE_DUMP( ( "  TPCC_RMSZ : %02d bytes\n", tpccRmsz ) );
	CIS_PARSE_DUMP( ( "  TPCC_RFSZ : %02d bytes\n", tpccRfsz ) );

	if( tupleLen < (tpccRasz+tpccRmsz+tpccRfsz+2) )
	{
		CIS_ERR( ( "CIS_ParseConfig: size error \n" ) );
	...
	CIS_PARSE_DUMP( ( "  Last Index Entry    : 0x%02X\n", pConfig->lastIndex ) );
	...
	CIS_PARSE_DUMP( ( "	Config Reg Base Addr: 0x%08X\n",	pConfig->baseAddr ) );
	...
	if( tupleBody & 0x40 )
	{
		...
		CIS_PARSE_DUMP( ( "  Interface Num       : 0x%04X\n", stciIfn ) );
...
static TPL_STATUS CIS_ParseCftableEntry(MOD_ID mId, UINT8 *pBuf)
{
	...
	if( tupleBody == STCE_PD )
	{
		...
		CIS_INFO( ( "  Physical Device String: %s\n", pStceEvpd->physicalDev ) );
	}
