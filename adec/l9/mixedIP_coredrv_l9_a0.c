...
static LX_ADEC_POWER_STATUS_T gADECPowerStatus_L9A0[LX_ADEC_POWER_MAX] = {	\
...
static LX_ADEC_CLOCK_SRC_T gAdecClockSource_L9A0 = LX_ADEC_CLOCK_UNKNOWN;

//Current SPDIF Sampling Frequency Setting
static LX_ADEC_SAMPLING_FREQ_T gAdecSpdifFreq_L9A0 = LX_ADEC_SAMPLING_FREQ_NONE;
...
int L9A0_ANALOG_ReadRegister(UINT8 slaveAddr, UINT8 regAddr, UINT8 *value)
{
	...
	*value = i2cData->data;
	...
	ADEC_MIP_DEBUG("Read  : Slave 0x%2x, reg 0x%2x  Data = 0x%2x\n", i2cData->slaveAddr, i2cData->regAddr, i2cData->data);
//	ADEC_ERROR("Read  : Slave 0x%2x, reg 0x%2x  Data = 0x%2x\n", i2cData->slaveAddr, i2cData->regAddr, i2cData->data);
...
int L9A0_ANALOG_WriteRegister(UINT8 slaveAddr, UINT8 regAddr, UINT8 value)
{
	...
	ADEC_MIP_DEBUG("Write : Slave 0x%2x, reg 0x%2x  Data = 0x%2x\n", i2cData->slaveAddr, i2cData->regAddr, i2cData->data);
//	ADEC_ERROR("Write : Slave 0x%2x, reg 0x%2x  Data = 0x%2x\n", i2cData->slaveAddr, i2cData->regAddr, i2cData->data);
...
int L9A0_MIXED_IP_ADEC_L9_SetClockSource( LX_ADEC_CLOCK_SRC_T ui8AdecClockSrc )
{
	...
	else if( ui8AdecClockSrc == LX_ADEC_CLOCK_ADC ) //ADC to SPK, SCART AAD bypass
	{
...
{
	ADEC_MIP_DEBUG("L9A0_MIXED_IP_ADEC_L9_SetPcmClockRate : Not Implemented!!!\n");
...
int L9A0_MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq ( LX_ADEC_SAMPLING_FREQ_T ui32SamplingFreq, LX_ADEC_SPDIF_DTO_RATE_T stSpdifDtoRate )
{
	...
	if( ui32SamplingFreq == LX_ADEC_SAMPLING_FREQ_48_KHZ )
	{
		...
		{
			ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq : same to 48_KHZ\n" );
			return RET_OK;
		}

		ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq : 48_KHZ\n" );
	...
	{
		ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq : 44_1KHZ\n" );
	...
	{
		ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq : 32_KHZ\n" );
	...
	else
	{
		...
		ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq : %d => 48KHz\n", ui32SamplingFreq);
	...
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_SetSPDIFSamplingFreq : %dKHz\n", ui32SamplingFreq);
...
{
	//Setting Mixed IP
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_InitCodecRegisters : Start\n");
	...
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_InitCodecRegisters : Done!!!\n");
...
//L9 A0 ADC uses AAD clock for operation, so AAD block must always enabled now.
	...
	{
		if( (gADECPowerStatus_L9A0[LX_ADEC_POWER_AAD].block == block)
		  &&(gADECPowerStatus_L9A0[LX_ADEC_POWER_AAD].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_L9A0[LX_ADEC_POWER_AAD].block = block;
	...
	{
		if( (gADECPowerStatus_L9A0[LX_ADEC_POWER_ADC].block == block)
		  &&(gADECPowerStatus_L9A0[LX_ADEC_POWER_ADC].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_L9A0[LX_ADEC_POWER_ADC].block = block;
	...
	{
		if( (gADECPowerStatus_L9A0[LX_ADEC_POWER_HPDRV].block == block)
		  &&(gADECPowerStatus_L9A0[LX_ADEC_POWER_HPDRV].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_L9A0[LX_ADEC_POWER_HPDRV].block = block;
	...
	{
		if( (gADECPowerStatus_L9A0[LX_ADEC_POWER_DAC].block == block)
		  &&(gADECPowerStatus_L9A0[LX_ADEC_POWER_DAC].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_L9A0[LX_ADEC_POWER_DAC].block = block;
	...
	{
		if( (gADECPowerStatus_L9A0[LX_ADEC_POWER_MIC].block == block)
		  &&(gADECPowerStatus_L9A0[LX_ADEC_POWER_MIC].mode  == mode ) )
		{
			//update power block
			gADECPowerStatus_L9A0[LX_ADEC_POWER_MIC].block = block;
...
{
	//Setting Mixed IP
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_L9_StartCodec : Start\n");
	...
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_DRV_StartUpCodec : Done\n");
...
{
	ADEC_DEBUG_TMP("MIXED_IP_ADEC_DRV_StopCodec : Not Implemented!!!\n");
...
int L9A0_MIXED_IP_ADEC_L9_SetADCPortNumber ( UINT8 ui8PortNum )
{
	...
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_DRV_SetADCPortNumber : ADC Port = %d\n", ui8PortNum);
...
int L9A0_MIXED_IP_ADEC_L9_SetDACOutVolume ( LX_ADEC_DAC_VOL_T stDACOutVol )
{
	...
	LX_ADEC_OUTPUT_PORT_MUTE_T portMute = {0, FALSE, FALSE, LX_ADEC_DAC_NONE};
	...
	if ( stDACOutVol.mode & LX_ADEC_DAC_SCART )
	{
		...
		if(dac1_mute_ena == 0)
		{
			...
			portMute.portNum  = LX_ADEC_OUTPUT_DAC0;
			portMute.mode	  = LX_ADEC_DAC_MIXEDIP;
			portMute.bHwOnOff = TRUE;
			(void)MIXED_IP_ADEC_L9_SetDACOutMute ( portMute );
		}
		...
		if ( stDACOutVol.mode == LX_ADEC_DAC_SCART )
		{
			ADEC_MIP_DEBUG("MIXED_IP_ADEC_DRV_SetDACOutVolume : SCART Gain = %d(dac0 = 0x%X, dac1 = 0x%X)\n", stDACOutVol.scartGain, gacodec_3.dac0_vol_con_1, gacodec_12.dac1_vol_con_1);
	...
	if ( stDACOutVol.portNum & LX_ADEC_OUTPUT_DAC0 )
	{
		...
		ADEC_MIP_DEBUG("SetDACOutVolume : SCART(dac1_0 = 0x%X, dac1_1 = 0x%X)\n", gacodec_13.dac1_vol_con_0, gacodec_12.dac1_vol_con_1);
	}
	...
	if ( stDACOutVol.portNum & LX_ADEC_OUTPUT_DAC2 )
	{
		...
		ADEC_MIP_DEBUG("SetDACOutVolume : HPDRV(dac0_0 = 0x%X, dac0_1 = 0x%X)\n", gacodec_4.dac0_vol_con_0, gacodec_3.dac0_vol_con_1);
	}

	ADEC_MIP_DEBUG("MIXED_IP_ADEC_DRV_SetDACOutVolume : Port = %d, Volume = 0x%X\n", stDACOutVol.portNum, stDACOutVol.hwVolume);
...
int L9A0_MIXED_IP_ADEC_L9_SetDACOutMute ( LX_ADEC_OUTPUT_PORT_MUTE_T stOutPortMute )
{
	...
	ADEC_MIP_DEBUG("MIXED_IP_ADEC_DRV_SetDACOutMute : Port = %d, mute = 0x%X\n", stOutPortMute.portNum, stOutPortMute.bHwOnOff);
