...
#include "../adec_common/adec_reg_api.h"

#if (ADEC_OPT_BUF_USE_REG == 1)

#define BUF_REG_READ(_ADDR_, _BITS_, _BIT_OFFSET_, _VAL_) \
	_VAL_ = (ADEC_REG_ReadRegister(_ADDR_) >> _BIT_OFFSET_) & (0xffffffff >> (32 - _BITS_));
#define BUF_REG_WRITE(_ADDR_, _BITS_, _BIT_OFFSET_, _VAL_)	\
	ADEC_REG_WriteRegister(_ADDR_, \
		(ADEC_REG_ReadRegister(_ADDR_) & (~((0xffffffff >> (32 - _BITS_ - _BIT_OFFSET_))<< _BIT_OFFSET_))) | \
