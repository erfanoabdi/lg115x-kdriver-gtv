...
int MJPEG_Init(void)
{
	...
	err = request_irq( L9_IRQ_ICOD1, (irq_handler_t)MJPEG_Interrrupt, 0, "L9-MJPEG", NULL );
	if (err)
	{
...
MJPEG_Close(struct inode *inode, struct file *file)
{
	...
	if ( my_dev->dev_open_count <= 0 )
	{
		...
		if ( _Finalize() != RET_OK ) { return -ENOMEM; }
...
MJPEG_Ioctl ( struct file *filp, unsigned int cmd, unsigned long arg )
{
	...
	switch(cmd)
	{
		...
		case LX_MJPEG_IOR_GET_BUFFER_INFO:
		{
			...
			stParam.addr = g_stMemCfgMJPEG.ui32Memory_base;
			stParam.length = g_stMemCfgMJPEG.ui32Memory_size;
		...
		case LX_MJPEG_IOW_ENCODE_INIT:
		{
			...
			my_dev->stOption.eQuality  = stInitParam->stOption.eQuality;
#endif
			OS_KFree(stInitParam);
...
MJPEG_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	start = g_stMemCfgMJPEG.ui32Memory_base & PAGE_MASK;
	len = PAGE_ALIGN((start & ~PAGE_MASK) + g_stMemCfgMJPEG.ui32Memory_size);
...
_Initialize(void)
{
	...
	ICOD_REG_VIRT = (UINT32)ioremap( ICOD_REG_BASE, ICOD_REG_SIZE );
...
//	g_MJPEG_device->stInnerStream.mem_buf.addr	 = g_stMemCfgMJPEG.ui32Memory_base;
//	g_MJPEG_device->stInnerStream.mem_buf.length = g_stMemCfgMJPEG.ui32Memory_size;
...
/**
 * Shadow register Finalize for MJPEG device.
 *
 */
static SINT32
_Finalize(void)
{
	if ( gpRealRegICOD != NULL && gpRealRegMJPEG != NULL )
	{
	...
	MJPEG_PRINT("mjpeg device finalized");
