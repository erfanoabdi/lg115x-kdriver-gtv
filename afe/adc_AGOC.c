	...
	do{
		...
		for(offset_adjust_count  =0; offset_adjust_count < 3; offset_adjust_count ++)
		{
			...
			if( ( (gain_ire73.gain_R >> 14) == 0) || ( (gain_ire73.gain_R >> 14) == 0) || ( (gain_ire73.gain_B >> 14) == 0) )
			{
				...
				{
					offset.offset_R += (1023 - (gain_ire950.gain_R>>14)) / 2;
				}
				if( (gain_ire73.gain_G>>14) == 0)
				{
					offset.offset_G += (1023 - (gain_ire950.gain_G>>14)) / 2;
				}
				if( (gain_ire73.gain_B>>14) == 0)
				{
					offset.offset_B += (1023 - (gain_ire950.gain_B>>14)) / 2;
				}

				ADC_DEBUG("Set Offset to R:[0x%x], G:[0x%x], B:[0x%x] \n" , offset.offset_R , offset.offset_G, offset.offset_B);
				ADC_SetOffsetRGB(&offset); // added by won.hur(won.hur@lge.com) @2011.08.17
				//ADC_L9Ax_SetOffsetRGB(&offset);

		//		ADC_SetIRELevel(IRE73 , &gain_ire73);
		//		ADC_DEBUG("Adjusted AGC gain R:73ire [%d], G:73ire [%d], B:73ire [%d] \n" , (int)(gain_ire73.gain_R) , (int)(gain_ire73.gain_G), (int)(gain_ire73.gain_B));
			}
			else
				break;
		}
		...
		for(offset_adjust_count  =0; offset_adjust_count < 3; offset_adjust_count ++)
		{
			...
			if( ( (gain_ire950.gain_R>>14) >= 1023) || ( (gain_ire950.gain_G>>14) >= 1023) || ( (gain_ire950.gain_B>>14) >= 1023) )
			{
				...
				if( (gain_ire950.gain_R>>14) >= 1023)
					offset.offset_R -= (gain_ire73.gain_R>>14) / 2;
				if( (gain_ire950.gain_G>>14) >= 1023)
					offset.offset_G -= (gain_ire73.gain_G>>14) / 2;
				if( (gain_ire950.gain_B>>14) >= 1023)
					offset.offset_B -= (gain_ire73.gain_B>>14) / 2;
		...
		if(bStatusR)
		{
			...
			*/
			...
			else
			{
				...
				AFE_PRINT("before gain R [0x%x]\n", (UINT32)temp);
				AFE_PRINT("GAP R[%d]\n", (int)gap.gain_R );
				AFE_PRINT("Target R[%d]\n", (int)target.gain_R);
	...
	//	(int)(dx) , (int)(( (float)dx - (float)z )*10000.0) )
		...
		//	AFE_PRINT("AGC gain G: 73ire %.5f , 950ire %.f\n" , gain_ire73.gain_G , gain_ire950.gain_G);
			...
			*/
			...
			else
			{
				...
				AFE_PRINT("before gain G [0x%x]\n",(UINT32)temp);
				AFE_PRINT("GAP G[%d]\n", (UINT32)gap.gain_G );
				AFE_PRINT("Target G[%d]\n", (UINT32)target.gain_G);
		...
		if(bStatusB)
		{
			...
			else
			{
				...
				AFE_PRINT("before gain B [0x%x]\n",(UINT32)temp);
				AFE_PRINT("GAP B[%d]\n", (UINT32)gap.gain_B );
				AFE_PRINT("Target B[%d]\n", (UINT32)target.gain_B);
...
/**
 *
 * search gain value for R,G,B(L8)
 *
 * @param	ST_GAIN_INFORM
 * @return	0 : OK , -1 : NOK
 *
*/
int ADC_L8_AGC_Measurement (LX_AFE_ADC_CALIB_INFO_T *calib_param, ST_GAIN_INFORM *gain)
{
	int ret = 0;
	S_GAIN_TARGET gain_ire73 , gain_ire950 , gap;
	static bool bStatusR,bStatusG,bStatusB;
	...
	max_gain_value = 0x3FF;			// For L8 10bit Gain
	...
	if( (calib_param->TargetForRGain > 0) || (calib_param->TargetForGGain > 0) || (calib_param->TargetForBGain > 0))
	{
		...
		target.gain_G = calib_param->TargetForGGain;
		target.gain_B = calib_param->TargetForBGain;
		AFE_PRINT("target R[%d], G[%d], B[%d]\n", (UINT32)target.gain_R, (UINT32)target.gain_G, (UINT32)target.gain_B);
	}
	else
	{
		...
		if(bStatusR)
		{
			...
			else
			{
				...
				AFE_PRINT("Target R[%d]\n", (int)target.gain_R);
					...
					if(target.gain_R > gap.gain_R)
						temp += (int)(target.gain_R - (int)(gap.gain_R - set_value));
					else
						temp += (int)(target.gain_R - (int)(gap.gain_R + set_value));

					gain_temp->gain_R = (unsigned int)temp;
	...
	//	(int)(dx) , (int)(( (float)dx - (float)z )*10000.0) )
		...
		//	AFE_PRINT("AGC gain G: 73ire %.5f , 950ire %.f\n" , gain_ire73.gain_G , gain_ire950.gain_G);
			...
			//120103 wonsik.do : removed floating point in kernel driver
			AFE_PRINT("AGC gain G: 73ire %d , 950ire %d\n" , (int)gain_ire73.gain_G ,(int)gain_ire950.gain_G );
			if(abs(target.gain_G - gap.gain_G) < GAP_TARGET)
			{
				bStatusG = 0;	//  done the adjust!!
				AFE_PRINT("G gain adjust done!!\n");
			...
			else
			{
				...
				AFE_PRINT("Target G[%d]\n", (int)target.gain_G);
					...
					if(target.gain_G > gap.gain_G)
						temp += (int)(target.gain_G - (int)(gap.gain_G - set_value));
					else
						temp += (int)(target.gain_G - (int)(gap.gain_G + set_value));

					gain_temp->gain_G = (unsigned int)temp;
		...
		if(bStatusB)
		{
			...
			gap.gain_B = abs(gain_ire73.gain_B - gain_ire950.gain_B);
			//AFE_PRINT("AGC gain B: 73ire %.5f , 950ire %.f\n" , gain_ire73.gain_B , gain_ire950.gain_B);
			AFE_PRINT("AGC gain B: 73ire %d , 950ire %d\n" , (int)gain_ire73.gain_B ,(int)gain_ire950.gain_B );
			if(abs(target.gain_B - gap.gain_B) < GAP_TARGET)
			{
				bStatusB = 0;	//  done the adjust!!
				AFE_PRINT("B gain adjust done!!\n");
			...
			else
			{
				...
				// set the value again
				temp = (int)gain_temp->gain_B;
				AFE_PRINT("before gain B [0x%x]\n", temp);
				AFE_PRINT("GAP B[%d]\n", (int)gap.gain_B );
				AFE_PRINT("Target B[%d]\n", (int)target.gain_B);

					if(target.gain_B > gap.gain_B)
						temp += (int)(target.gain_B - (int)(gap.gain_B - set_value));
					else
						temp += (int)(target.gain_B - (int)(gap.gain_B + set_value));

					gain_temp->gain_B = (unsigned int)temp;
	...
	if(iRepeatCount > ADC_AGC_NUM)
	{
		ret = -1;

	}
	AFE_PRINT("result R[%x] G[%x] B[%x]\n", gain_temp->gain_R, gain_temp->gain_G,gain_temp->gain_B);
	memcpy( (void *)gain , (void *)gain_temp , sizeof(ST_GAIN_INFORM));
...
//	AFE_PRINT("%s entered with index[%d]\n", __func__ , iIndex);
	...
	if ( lx_chip_rev( ) >= LX_CHIP_REV( L9, A0 ) )
		...
		ret = ADC_L8_AGC_Measurement(calib_param, &gain);
