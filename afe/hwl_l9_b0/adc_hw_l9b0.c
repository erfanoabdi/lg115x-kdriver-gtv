...
//	ADC_DEBUG("IRE level set %d\n",ire);
	...
	// set target value
	if(ire == IRE0_COMP){
		gadc_dig_90_bx.r_compare_1 = ((512<<14)&0x300)>>8;
		gadc_dig_91_bx.r_compare_0 = ((512<<14)&0xFF);
		VPORT_I2C_Write((UINT32*)&gadc_dig_90_bx);
		VPORT_I2C_Write((UINT32*)&gadc_dig_91_bx);

		gadc_dig_92_bx.g_compare_1 = ((target_ire<<14)&0x300)>>8;
		gadc_dig_93_bx.g_compare_0 = ((target_ire<<14)&0xFF);
		VPORT_I2C_Write((UINT32*)&gadc_dig_92_bx);
		VPORT_I2C_Write((UINT32*)&gadc_dig_93_bx);

		gadc_dig_94_bx.b_compare_1 = ((512<<14)&0x300)>>8;
		gadc_dig_95_bx.b_compare_0 = ((512<<14)&0xFF);
		VPORT_I2C_Write((UINT32*)&gadc_dig_94_bx);
		VPORT_I2C_Write((UINT32*)&gadc_dig_95_bx);
	}
	else
	{
		gadc_dig_90_bx.r_compare_1 = ((target_ire<<14)&0x300)>>8;
		gadc_dig_91_bx.r_compare_0 = ((target_ire<<14)&0xFF);
		VPORT_I2C_Write((UINT32*)&gadc_dig_90_bx);
		VPORT_I2C_Write((UINT32*)&gadc_dig_91_bx);
		...
		gadc_dig_94_bx.b_compare_1 = ((target_ire<<14)&0x300)>>8;
		gadc_dig_95_bx.b_compare_0 = ((target_ire<<14)&0xFF);
		VPORT_I2C_Write((UINT32*)&gadc_dig_94_bx);
		VPORT_I2C_Write((UINT32*)&gadc_dig_95_bx);
	}
...
int ADC_L9Bx_Check_Htotal_Diff(UINT8 count, int Kadp_Index)
{
	...
	for(i = 0; i < count;i++)
	{
		...
		if(abs(maxCnt16 - hsoutprd16) > 0)
		{
			...
			if(hsoutprd16 > 1)
				ADC_Reset_Digital();
			else if(hsoutprd16 == 1) 	//2012.01.26 : Added for TVIX component error
