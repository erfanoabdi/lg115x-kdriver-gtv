...
//	OS_DEBUG_DisableModule ( g_afe_debug_fd );
	...
	OS_DEBUG_EnableModuleByIndex ( g_afe_debug_fd, 0, DBG_COLOR_NONE );
//	OS_DEBUG_EnableModuleByIndex ( g_afe_debug_fd, 1, DBG_COLOR_NONE );
//	OS_DEBUG_EnableModuleByIndex ( g_afe_debug_fd, 2, DBG_COLOR_NONE );
	OS_DEBUG_EnableModuleByIndex ( g_afe_debug_fd, 3, DBG_COLOR_NONE );
...
//	AFE_TRACE("cmd = %08X (cmd_idx=%d)\n", cmd, _IOC_NR(cmd) );
	...
	switch(cmd)
	{
		...
		case AFE_IOW_SET_HSYNC_DETECTION_FOR_TUNING:
		{
			...
			CVD_DEBUG("CVD SET Hsync detection for tuning\n");
		...
		case AFE_IOW_CVD_SET_VBI_TYPE:
			ADC_DEBUG("AFE Set VBI Type\n");

			ret = CVD_Set_VBI_Type(arg);
			break;
		...
		case AFE_IOW_CVD_VDAC_POWER_CONTROL:
			{
				...
				if ( lx_chip_rev( ) >= LX_CHIP_REV( L9, B0 ) )
					ret =	CVD_L9Bx_OnOff_VDAC(bVDAC_Power_Control);
				else if ( lx_chip_rev( ) >= LX_CHIP_REV( L9, A0 ) )
					ret =	CVD_L9Ax_OnOff_VDAC(bVDAC_Power_Control);
			}
			break;

		case AFE_IOW_LVDS_SRC_CONTROL_T:
			{
				...
				if ( lx_chip_rev( ) >= LX_CHIP_REV( L9, B0 ) )
					ret =	ADC_L9Bx_LVDS_Src_Control(&LVDS_Control_t);
			}
			break;
