...
extern int  nandsim_process_mtd_map_file(struct mtd_info* mtd, char* map_file );
extern int  nandsim_proc_mtdinfo_init(struct mtd_info *mtd);
extern void nandsim_proc_mtdinfo_exit(void);
#endif
...
#ifdef SUPPORT_LG_GP2_NAND
module_param(map_file,      charp, 0400);
module_param(page_file, charp, 0400);
#endif
...
#ifdef SUPPORT_LG_GP2_NAND
MODULE_PARM_DESC(map_file,       "File to use to flash map for supporting LG GP2 ");
MODULE_PARM_DESC(page_file,		 "File to use to page  map for supporting LG GP2");
#endif
...
#ifdef SUPPORT_LG_GP2_NAND
	struct file *pfile; /* page file */
	unsigned char* pfile_pages_cache;
#endif
...
#ifdef SUPPORT_LG_GP2_NAND
/* low  level interface for page file */
static struct file* pagefile_vfs_open ( char* filename )
{
	struct file* fd = filp_open(filename, O_CREAT | O_RDWR | O_LARGEFILE, 0600);

	return fd;
}

/* low  level interface for page file */
static void		pagefile_vfs_close ( struct file* fd )
{
	if ( fd )
	{
		filp_close ( fd, NULL );
...
/* low  level interface for page file */
static ssize_t pagefile_vfs_read	( struct file* fd, char* buf, size_t count, loff_t* pos )
{
	int	memalloc;
	mm_segment_t old_fs;
	ssize_t rx = 0;		
	...
	if ( fd )
	{
		...
		rx = vfs_read(fd, (char __user *)buf, count, pos );
		clear_memalloc(memalloc);
		set_fs(old_fs);

		if ( rx != count )
		{
			printk("<warning> %s: insufficient data read. requested size = %d, actual read size = %d\n", 
							__FUNCTION__, count, rx );
		}
	}
...
/* low  level interface for page file */
static ssize_t pagefile_vfs_write	( struct file* fd, const char* buf, size_t count, loff_t* pos)
{
	int	memalloc;
    mm_segment_t old_fs;
    ssize_t tx = 0;
    ...
    if ( fd )
    {
        ...
        set_fs(get_ds());
		memalloc = set_memalloc();
        tx = vfs_write(fd, (const char __user *)buf, count, pos );
		clear_memalloc(memalloc);
        set_fs(old_fs);

        if ( tx != count )
        {
            printk("<warning> %s: insufficient data write. requested size = %d, actual read size = %d\n",
                            __FUNCTION__, count, tx );
        }
    }
...
static void   load_pagefile ( struct nandsim* ns, char* filename )
{
	loff_t pos;
	...
	if (filename)
	{
		pagefile = pagefile_vfs_open( filename );

		if (pagefile)
		{
			pos = 0;
			printk("<!> %s: loadng %d bytes\n", __FUNCTION__, ns->geom.pgnum );
			pagefile_vfs_read ( pagefile, ns->pages_written, ns->geom.pgnum, &pos );
		}
		ns->pfile = pagefile;

		/* alloc page cache */
#if 0
		ns->pfile_pages_cache = vmalloc(ns->geom.pgnum);
		memcpy( ns->pfile_pages_cache, ns->pages_written, ns->geom.pgnum );
#endif
	}
}

static void   save_pagefile ( struct nandsim *ns )
{
	loff_t	pos;

	if ( ns->pfile ) 
	{
		/* check if pages_written is changed or not */
#if 0
		if ( !memcmp( ns->pfile_pages_cache, ns->pages_written, ns->geom.pgnum ) )
		{
			return;
		}
		memcpy ( ns->pfile_pages_cache, ns->pages_written, ns->geom.pgnum );
#endif
		pos = 0;
		pagefile_vfs_write ( ns->pfile, ns->pages_written, ns->geom.pgnum, &pos );
	}
}
...
#ifdef SUPPORT_LG_GP2_NAND
	/* raxis.lim@lge.com (2009/10/26) - process LG GP2 flash map */
	nandsim_proc_mtdinfo_init(nsmtd);

	if ( page_file )
	{
		load_pagefile ( nand, page_file );
	}

	if ( map_file )
	{
		nandsim_process_mtd_map_file( nsmtd, map_file );
	}
	else
#endif
	{
